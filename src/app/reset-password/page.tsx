...
function ResetPasswordContent() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (password.length < PASSWORD_MIN_LENGTH) return setError(`Пароль должен быть не короче ${PASSWORD_MIN_LENGTH} символов.`);
  ...
  return <main className={styles.page}>
    ...
    <div className={styles.shell}>
      ...
      <section className={styles.card}>
        {success ? <div className={styles.successCard}><span className={styles.successIcon}><CheckCircle2 size={28} /></span><h1>Пароль изменён</h1><p>Теперь войдите с новым паролем.</p><p className={styles.footer}><Link href="/login">Перейти ко входу <ArrowRight size={14} /></Link></p></div> : <><header className={styles.cardHeader}><span className={styles.kicker}>Восстановление</span><h1>Задайте новый пароль</h1><p>Минимум {PASSWORD_MIN_LENGTH} символов. Не используйте пароль от Roblox.</p></header><form className={styles.form} onSubmit={submit}>
          <div className={styles.label}><label htmlFor="reset-password">Новый пароль</label><span className={styles.field}><Lock size={18} /><input id="reset-password" className={styles.input} type={show ? "text" : "password"} autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} required minLength={PASSWORD_MIN_LENGTH} /><button type="button" className={styles.iconButton} onClick={() => setShow((value) => !value)} aria-pressed={show} aria-label={show ? "Скрыть пароль" : "Показать пароль"}>{show ? <EyeOff size={18} /> : <Eye size={18} />}</button></span></div>
          <div className={styles.label}><label htmlFor="reset-password-confirm">Повторите пароль</label><span className={styles.field}><Lock size={18} /><input id="reset-password-confirm" className={styles.input} type={show ? "text" : "password"} autoComplete="new-password" value={confirm} onChange={(event) => setConfirm(event.target.value)} required minLength={PASSWORD_MIN_LENGTH} /></span></div>
