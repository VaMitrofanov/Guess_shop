...
function OrderDetail({ order, data, busy, manualCode, message, setManualCode, setMessage, onBack, onAction }: {
  ...
  return <div className={css.detailScreen}>
    ...
    {!order.lastErrorCode && order.blockedReason && <div className={css.mobileError}><AlertTriangle /><div><strong>Действия недоступны</strong><span>{order.blockedReason}</span></div></div>}
    ...
    {order.stage === "complete" && <div className={css.completeCard}><CheckCircle2 /><div><strong>Заказ завершён</strong><span>Код получения удалён, цифровая выдача продолжается по обычному сценарию.</span></div></div>}
    ...
    <section className={css.mobileManual}><div><strong>Резервный ввод 5–6 цифр</strong><small>Код отобразится после сохранения</small></div><div><input value={manualCode} onChange={(event) => setManualCode(event.target.value.replace(/\D/g, "").slice(0,6))} inputMode="numeric" autoComplete="off" maxLength={6} placeholder="123456" /><button disabled={!order.permissions.saveDeliveryCode || manualCode.length < 5 || Boolean(busy)} onClick={() => onAction("save_delivery_code", { code: manualCode })}>Сохранить</button></div></section>
