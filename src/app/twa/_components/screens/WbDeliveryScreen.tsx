...
function OrderDetail({ order, data, busy, manualCode, message, setManualCode, setMessage, onBack, onAction }: {
  ...
  return <div className={css.detailScreen}>
    ...
    <section className={css.mobileManual}><div><strong>Резервный ввод 6 цифр</strong><small>Код отобразится после сохранения</small></div><div><input value={manualCode} onChange={(event) => setManualCode(event.target.value.replace(/\D/g, "").slice(0,6))} inputMode="numeric" autoComplete="off" maxLength={6} placeholder="••••••" /><button disabled={!order.permissions.saveDeliveryCode || manualCode.length !== 6 || Boolean(busy)} onClick={() => onAction("save_delivery_code", { code: manualCode })}>Сохранить</button></div></section>
