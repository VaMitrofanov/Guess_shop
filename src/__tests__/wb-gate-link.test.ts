...
describe("WB gate link handed to the buyer", () => {
  ...
  /* Третье напоминание (неделя) появилось 12.09.2026: до него бот замолкал
     после суток НАВСЕГДА. Оба возврата, которые мы получили, пришли из этого
     хвоста, и оба с одним текстом — «не знаю, как сделать геймпасс». Значит
     повторять инструкцию бессмысленно: человек её видел. */
  it("the week-old nudge offers help instead of repeating the instruction", () => {
    const third = wbGateReminderMessage("QUN5YFZ", 500, 3);
