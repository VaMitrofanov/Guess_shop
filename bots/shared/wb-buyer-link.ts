 ...
 *
 * Было три часа (решение владельца О5, 20.08.2026) — оценка «сколько ждать»,
 * сделанная до того, как появились данные. Данные на 07.09.2026, 64 закрытые
 * доставки: 60 покупателей приходят в первые 3 часа, ещё 3 — до суток, один —
 * через 45 часов. То есть окно в три часа отсекало каждого шестнадцатого, и
 * каждый такой случай превращался в ручную переписку.
 *
 * Сутки. Перебор от этого проще не становится: код пятизначный, но попыток
 * даётся три в час на человека (`allowDeliveryCodeAttempt`), то есть 72 попытки
 * за окно на пространстве в 100 000 — и ответ бота одинаков при попадании и при
 * промахе, так что подсказки из него не выжать. За сутками — человек: там уже
 * не «покупатель не дошёл», а разбор.
 */
export const AUTO_LINK_WINDOW_MS = 24 * 60 * 60_000;
