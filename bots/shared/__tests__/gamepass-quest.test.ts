...
describe("экран результата", () => {
  ...
  test("достраиваем: свой пасс в списке, недостающий — задачей", () => {
    // 1500 закрывается пассом на 2143; до 2000 не хватает 500 — их создаём.
  ...
  test("некруглый номинал (1200) закрывается ОДНИМ пассом, а не 500 + 700", () => {
    // Номиналы 300/800/1200 в каталоге ВБ на 500 не делятся, зато целиком
    // помещаются в одного донора. Пасс покупателя на 500 тут не помощник:
    // остаток 700 — как раз тот огрызок, из-за которого доноры застревают.
    const plan = planFromOwned(1200, [pass("1", 715)]);
    expect(plan.kind).toBe("empty");
    expect(targetsToCreate(plan).map((t) => t.amount)).toEqual([1200]);
  });
...
describe("что создаём по ключу", () => {
  test("эталонный набор под номинал, а не «чего не хватает»", () => {
    ...
    // (14 на руки). Разбор плана его засчитывал и просил ОДИН пасс на 2838 —
    // выходила пара «2838 + 20»: первый выкупается только с крупного донора,
    // второй — отдельный поход к донору ради 14 робуксов. С 13.09.2026 мелочь
    // не проходит и в план: часть обязана быть кратна 500 и влезать в донора.
    const junk = planFromOwned(2000, [pass("junk", 20, "Мелочь")]);
    expect(targetsToCreate(junk).map((t) => t.price)).toEqual([2143, 715]);
