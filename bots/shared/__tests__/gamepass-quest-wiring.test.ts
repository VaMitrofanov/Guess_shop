...
describe.each([
  ...
  test("по ключу создаётся эталонный набор, а не «чего не хватает»", () => {
    // Иначе повторяется случай 07.09.2026: пасс на 20 R$ на аккаунте → заказ
    // из «2838 + 20» вместо «2143 + 715».
    expect(source).toContain("createTargetsFor(quest.denomination)");
  });
