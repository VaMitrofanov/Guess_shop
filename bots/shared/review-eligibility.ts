...
export function reviewIneligibleMessage(
  ...
  if (elig.kind === "active_order") {
    return (
      ...
      `Скрин отзыва пришли после выкупа — получишь ${b(`+${REVIEW_BONUS_AMOUNT} R$`)} к следующей прямой покупке в боте или на robloxbank.ru. Отдельно бонус не выдаётся.`
    );
  }
  ...
  if (elig.grantedAt && elig.expiresAt) {
    ...
    return (
      ...
      balanceLine +
      `Он автоматически добавится к следующей прямой покупке — просто оформи заказ в боте или на robloxbank.ru. К покупке на WB бонус не применяется.`
    );
  }

  return (
    `✅ Бонус за отзыв по этому заказу уже был начислен — повторно за тот же заказ он не начисляется.\n\n` +
    `После следующего выполненного заказа новый отзыв даст ещё +${REVIEW_BONUS_AMOUNT} R$ к следующей прямой покупке 😉`
  );
}
