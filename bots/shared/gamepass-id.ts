...
/**
 * Голый ID геймпасса. Пространство asset ID у Roblox монотонно растёт, поэтому
 * длина числа — рабочая проверка правдоподобия: пассы, которые создают прямо
 * сейчас, лежат около 1 900 000 000 (10 цифр), самые старые живые — 8 цифр.
 * Всё, что короче семи цифр, у нас на практике означает не пасс, а цену (715),
 * номинал (500) или обрывок номера — и такое число нельзя пускать дальше:
 * оно либо не найдётся, либо найдётся ЧУЖИМ пассом, а его владельцем мы
 * пометим заказ (`noteProbableNickByCode`).
 */
const BARE_ID_RE = /^\d{7,12}$/;
