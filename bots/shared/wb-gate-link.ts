 ...
 *
 * Просьба прислать код ТЕКСТОМ — не украшение: код вынимается из текста
 * сообщения (`extractDeliveryCode`), а присланный скриншот для нас пустой, и
 * заказ молча встаёт до ручного разбора. Пример показан в обоих написаниях,
 * потому что WB показывает код с пробелом, а вводить его привычно слитно —
 * принимаются оба, и выбор написания не должен становиться ещё одним вопросом.
 *
 * Длина названа диапазоном 5–7: WB отгружал коды всех трёх длин, и покупатель
 * с семизначным кодом, прочитавший «5-6-значный», решает, что у него не тот
 * код. */
...
/** WB пока не принимает код, но мы всё ещё пробуем.
 *
 * Отдельно от `wbCodeRetryMessage`: там мы сдались и ждём другой код, здесь —
 * работа продолжается. 22.08 покупатель получил «код не подошёл» через восемь
 * минут, хотя тот же самый код WB принял через четыре часа; сообщение, которое
 * выглядит как тупик, отправляет человека в поддержку и в отзыв. Просьба
 * перепроверить код остаётся — опечатку исправит только покупатель, — но она
 * не главная мысль сообщения. */
export function wbCodeRecheckMessage(): string {
  return [
    "Мы получили ваш код и продолжаем закрывать доставку — Wildberries пока его не принимает,"
    + " так бывает при задержках на их стороне.",
...
export function wbGateReminderMessage(
  ...
  const amount = denomination ? `${denomination.toLocaleString("ru-RU")} R$` : "ваш номинал";
  /* Третье напоминание (неделя) говорит не то же самое третий раз, а другое.
     Оба возврата, которые мы получили, пришли из этого хвоста, и оба с одним
     текстом: «не знаю, как сделать геймпасс». Значит повторять инструкцию
     бессмысленно — человек её видел. Работает здесь только предложение
     сделать это вместе, поэтому ссылка уходит на второй план, а чат — на
     первый. */
  const opening = level === 1
    ...
    : level === 2
      ...
      : `Ваши ${amount} по-прежнему ждут вас — заказ никуда не делся и не сгорит.`;
  if (level >= 3) {
    return [
      opening,
      ...wbSiblingLines(sibling),
      "Чаще всего люди останавливаются на одном шаге — создать геймпасс в Roblox."
