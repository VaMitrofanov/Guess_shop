...
const VK_FAQ_ITEMS: { key: string; label: string; answer: string }[] = [
  ...
  { key: "managed",   label: "⚠️ Managed pricing?",         answer: "Managed pricing (региональные цены) должен быть ВЫКЛЮЧЕН.\n\nЕсли он включён — Roblox автоматически меняет цену геймпасса для разных стран, и выдача робуксов задержится.\n\nПроверь: Passes → твой пасс → ☰ → Sales → переключатель Managed pricing = OFF.\n\nПо умолчанию он выключен, но если случайно включил — выключи и нажми Save Changes." },
];
...
async function handleGamepassLink(
    ...
    if (Math.abs(gamepassInfo.price - expectedPrice) > 2) {
      await ctx.reply({
        message:
          ...
          `💡 Если у тебя включён Managed pricing — обязательно выключи его (Passes → твой пасс → ☰ → Sales → переключатель Managed pricing = OFF), иначе цена будет неверной и выдача робуксов задержится.\n\n` +
  ...
  await ctx.reply({
    message:
      ...
      `⚠️ Убедись, что Managed pricing выключен (Sales → переключатель OFF), иначе выдача задержится.\n\n` +
