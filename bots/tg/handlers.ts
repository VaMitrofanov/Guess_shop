...
import { priceMismatchText } from "../shared/direct-gamepass-copy";
...
import {
  ...
  mergeOwned,
  readyParts,
  type DirectPlanScreen,
} from "../shared/direct-plan";
import { intentPartsRows } from "../shared/gamepass-acceptance";
import { passFitsAmount } from "../shared/direct-requote";
import { loadRobloxApiKeyForUser, rememberRobloxApiKey, type RobloxApiKeyClient } from "../shared/roblox-api-key-store";
...
const FAQ_ITEMS: { key: string; label: string; answer: string; buttons?: { label: string; cb: string }[][] }[] = [
  ...
  { key: "managed",   label: "⚠️ Managed pricing?",         answer: "<b>Managed pricing</b> (региональные цены) должен быть <b>ОТКЛЮЧЁН</b>.\n\nЕсли он включён — Roblox автоматически меняет цену геймпасса и мы <b>не сможем</b> его выкупить. Робуксы ты получишь только когда всё будет сделано правильно.\n\nПроверь: Passes → твой пасс → ☰ → Sales → переключатель <b>Managed pricing = OFF</b>.\n\nПо умолчанию он отключён, но если случайно включил — отключи и нажми <b>Save Changes</b>." },
];
      ...
      if (!isAdmin) {
        ...
        if (startUser) {
          ...
          if (awaitingOrder) {
            ...
            await ctx.reply(
              `С возвращением! 👋 Заказ на <b>${awaitingOrder.amount} R$</b> ждёт геймпасс${startNickLine}\n\n` +
              `🔎 Начнём с ника Roblox: посмотрю, что у тебя уже выставлено — часто заказ собирается из готового и создавать ничего не нужно.\n` +
              `Если создавать всё-таки придётся — покажу, что именно, и дам выбрать способ.\n\n` +
              `🔔 Здесь же придут <b>уведомления о заказе</b> — приняли → выкупаем → готово.`,
              {
                parse_mode: "HTML",
                link_preview_options: { is_disabled: true },
                ...Markup.inlineKeyboard([
                  ...startSearchBtns,
                  [Markup.button.url("📖 Открыть инструкцию", startGuideUrl)],
                ]),
              }
            );
            return;
...
async function showSummary(ctx: any, flow: DirectFlowState, gpRobux: number, gpName: string, editTarget?: { chatId: number; messageId: number }): Promise<void> {
  ...
  /* Набор из нескольких пассов (как коридор ВБ): каждая часть сверена со своим
     номиналом при разборе, общая «цена пасса» к набору неприменима. */
  const set = flow.parts && flow.parts.length > 1 ? flow.parts : null;
  let mpLine = "";
  if (!set) try {
  ...
  const mismatch = !set && expectedGp > 0 && Math.abs(gpRobux - expectedGp) > 2;
  ...
  if (mismatch) {
    ...
    // Ключ — только когда метод включён: иначе кнопка вела в «способ недоступен».
  ...
  const summaryText =
    ...
    (set
      ? `🧩 Соберём из ${set.length} геймпассов — каждую часть выкупаем отдельно:\n${directPartsLines(set.map((p) => ({ ...p, name: escapeHtml(p.name) }))).join("\n")}${discountLine}\n`
...
async function handleDirectNickResolved(bot: Telegraf, ctx: any, nick: string): Promise<void> {
  ...
  // ── Дальше решает разбор плана, как в коридоре ВБ ────────────────────────
  // До 24.09.2026 здесь искался ОДИН пасс ровно на всю сумму: пак 2000 требовал
  // пасс за 2858, который не выкупит ни один донор, а выставленные пассы
  // покупателя не засчитывались. Теперь заказ собирается из того, что есть, и
  // просит создать только недостающее — тем же модулем, что квест ВБ.
  flow.gamesVisibility = result.games?.visibility ?? null;
  const owned = result.status === "no_gamepasses" ? [] : ownedFromSearch(result.all);
  await showDirectPlan(ctx, flow, owned, { chatId: ctx.chat.id, messageId: searchingMsg.message_id });
}

/** Кнопки экрана прямого плана → callback'и Telegram. */
function directPlanKb(screen: DirectPlanScreen) {
  return Markup.inlineKeyboard(screen.rows.map((row) => row.map((button) => {
    switch (button.action) {
      case "guide": return Markup.button.url(button.label, button.url ?? "https://robloxbank.ru/guide?source=direct");
      case "keyStored": return Markup.button.callback(button.label, CB.directKeyStored);
      case "key": return Markup.button.callback(button.label, CB.directKey);
      case "pick": return Markup.button.callback(button.label, CB.directGpPick(button.gamepassId ?? ""));
      case "nick": return Markup.button.callback(button.label, CB.directNickNew);
      case "back": return Markup.button.callback(button.label, CB.directBack);
      default: return Markup.button.callback(button.label, CB.directCancel);
    }
  })));
}

type EditTarget = { chatId: number; messageId: number };
/** Контекст Telegraf, с которым работает итог прямого заказа (`showSummary`). */
type DirectCtx = Parameters<typeof showSummary>[0];
/** Клиент хранилища ключей поверх общего `db` ботов. */
const keyStore = db as unknown as RobloxApiKeyClient;

/** Показать экран: переписать пузырь «Ищу…», если он есть, иначе новым сообщением. */
async function directShow(ctx: DirectCtx, text: string, extra: Record<string, unknown>, editTarget?: EditTarget): Promise<void> {
  if (editTarget) {
    try {
      await ctx.telegram.editMessageText(editTarget.chatId, editTarget.messageId, undefined, text, extra);
      return;
    } catch { /* сообщение устарело — ниже новым */ }
  }
  await ctx.reply(text, extra);
}

/**
 * Готовый план → итог прямого заказа. Одна часть — обычный пасс; несколько —
 * набор (ляжет в `DirectIntent.parts`, потом в `WbOrderGamepass`).
 */
async function applyDirectParts(ctx: DirectCtx, flow: DirectFlowState, parts: PlanPart[], editTarget?: EditTarget): Promise<void> {
  const head = parts[0];
  flow.parts = parts.length > 1 ? parts : undefined;
  flow.gamepassId = head.gamepassId;
  flow.gamepassUrl = `https://www.roblox.com/game-pass/${head.gamepassId}`;
  flow.gamepassName = head.name;
  flow.gamepassRobux = head.price;
  flow.requote = undefined;
  flow.step = "summary";
  await showSummary(ctx, flow, head.price, head.name, editTarget);
}

/**
 * Разбор аккаунта в прямом заказе: готово — итог, нет — экран «чего не хватает»
 * теми же словами, что квест ВБ. Пока экран открыт, текстом принимается ник,
 * ссылка или Pass ID.
 */
async function showDirectPlan(ctx: DirectCtx, flow: DirectFlowState, owned: OwnedPass[], editTarget?: EditTarget): Promise<void> {
  flow.owned = owned;
  const total = flow.totalAmount ?? 0;
  const plan = directPlan(total, owned);
  const parts = readyParts(plan);
  if (parts) {
    await applyDirectParts(ctx, flow, parts, editTarget);
    return;
  }
  flow.parts = undefined;
  flow.step = "nick_input";
  const nick = flow.robloxUsername ?? "";
  const keyEnabled = gamepassAutocreateEnabled();
  const userId = keyEnabled && nick ? await dbUserIdFor(ctx) : null;
  const storedKey = userId ? await hasStoredKeyFor(keyStore, userId, nick) : false;
  const screen = directNeedsScreen({
    totalAmount: total,
    ...
    keyEnabled,
    storedKey,
    gamesVisibility: flow.gamesVisibility,
    bonus: flow.bonus,
    guideUrl: `https://robloxbank.ru/guide?source=direct${nick ? `&username=${encodeURIComponent(nick)}` : ""}`,
  });
  const header = stepBar(...dirStep(flow, "gamepass"), "Геймпасс");
  await directShow(ctx, `${header}\n\n${screen.text}`, {
    parse_mode: "HTML",
    link_preview_options: { is_disabled: true },
    ...directPlanKb(screen),
  }, editTarget);
...
/**
 * Итог работы ключа в прямом заказе: созданные пассы достраивают план того же
 * аккаунта. Отказ — своими кнопками прямого заказа (кнопки квеста ВБ вели бы
 * в чужой флоу, где нет ни кода, ни заказа).
 */
async function finishDirectKey(
  ctx: DirectCtx,
  flow: DirectFlowState,
  nick: string,
  outcome: { created: { gamePassId: number; priceInRobux: number; name?: string }[]; error?: string },
  editTarget?: EditTarget,
): Promise<void> {
  if (outcome.created.length === 0) {
    const verdict = keyCreateVerdict(outcome.error);
    await directShow(ctx, `❌ <b>${verdict.title}</b>\n\n${verdict.text}`, {
      parse_mode: "HTML",
      ...Markup.inlineKeyboard([
        [Markup.button.callback(verdict.retry ? "🔁 Попробовать ещё раз" : "🔑 Прислать другой ключ", CB.directKey)],
        [Markup.button.url("📖 Создам сам (инструкция)", "https://robloxbank.ru/guide?source=direct")],
        [Markup.button.callback("◀️ Назад", CB.directBack), Markup.button.callback("❌ Отменить", CB.directCancel)],
      ]),
    }, editTarget);
    return;
  }
  const created: OwnedPass[] = outcome.created.map((pass) => ({
    gamepassId: String(pass.gamePassId),
    name: pass.name || `Пасс ${pass.priceInRobux}`,
    price: pass.priceInRobux,
    isForSale: true,
  }));
  const sameAccount = (flow.robloxUsername ?? "").toLowerCase() === nick.toLowerCase();
  flow.robloxUsername = nick;
  await directShow(ctx, `✅ <b>Сделали за тебя</b>\n\n${keyCreateSuccessText(outcome.created.map((p) => p.priceInRobux), nick)}`, { parse_mode: "HTML" }, editTarget);
  await showDirectPlan(ctx, flow, mergeOwned(sameAccount ? flow.owned ?? [] : [], created));
}

/** Открыть ветку ключа прямого заказа: эталонный набор под сумму, как у квеста ВБ. */
async function startDirectKey(ctx: DirectCtx, flow: DirectFlowState): Promise<void> {
  const targets = directKeyTargets(flow.totalAmount ?? 0);
  ...
  pendingDirectKey.set(ctx.from.id, { nick: flow.robloxUsername!, targets: targets.map((t) => t.price) });
...
async function handleDirectGamepassLink(ctx: DirectLinkCtx, passId: string): Promise<void> {
  ...
  // Правило коридора ВБ: для заказа нужен только Pass ID. Пасс достраивает
  // план ЭТОГО аккаунта (найденные по нику пассы того же владельца остаются),
  // получатель — владелец пасса.
  const edit = { chatId: ctx.chat.id, messageId: checkingMsg.message_id };
  const linked: OwnedPass = { gamepassId: passId, name: gp.name, price: gp.price, isForSale: true };
  const sameAccount = (flow.robloxUsername ?? "").toLowerCase() === owner.toLowerCase();
  const owned = mergeOwned(sameAccount ? flow.owned ?? [] : [], [linked]);
  flow.robloxUsername = owner;
  flow.owned = owned;
  const plan = directPlan(flow.totalAmount ?? 0, owned);
  const parts = readyParts(plan);
  if (parts) {
    await applyDirectParts(ctx, flow, parts, edit);
    return;
  }
  // Пасс не под этот объём и заказ им не достроить — честный пересчёт под него
  // («Едем на N R$»), как при выборе пасса из списка.
  if (plan.kind !== "build" && !passFitsAmount(gp.price, flow.totalAmount ?? 0)) {
    flow.parts = undefined;
    ...
    flow.step = "summary";
    await showSummary(ctx, flow, gp.price, gp.name, edit);
    return;
  }
  await showDirectPlan(ctx, flow, owned, edit);
}
...
/**
 * Buyer "mini profile" / home hub — the place a customer lands once the WB flow
 * is done, so the bot becomes the habit for the next (direct) purchase.
 * Loyalty-aware: shows tier, bonus balance, order count and the latest order at
 * a glance, plus the everyday actions (my order / buy directly / guide / help).
 * Fail-open: any DB hiccup still returns a usable menu.
 */
async function buildBuyerMenu(tgId: string, name?: string): Promise<StatusMessage> {
  ...
  try {
    ...
    console.error("[TG] buildBuyerMenu lookup failed:", err);
  }
  ...
  if (activeOrders.length > 0) {
    ...
    for (const o of activeOrders) {
      ...
      // Идентификатор для клиента = сумма+статус (для WB видит код в статусе).
  ...
  if (lastCompleted) {
    ...
    lines.push(`✅ Последний выполненный: ${lastCompleted.amount} R$ · ${dt}`);
  }
...
export function registerText(bot: Telegraf): void {
  ...
  bot.on("text", async (ctx) => {
    ...
    // 1y. Ветка ключа: следующий текст — Open Cloud ключ покупателя.
    ...
    if (!isAdmin && pendingApiKey.has(ctx.from.id)) {
      ...
      if (sweep === "valid") {
        await handleApiKeyInput(bot, ctx, text);
        return;
      }
      // "cleared" — прислали валидный код ВБ: падаем дальше, к штатному маршруту.
      pendingApiKey.delete(ctx.from.id);
    }
      ...
      if (payUser?.tgId) {
        try {
          await bot.telegram.sendMessage(
            payUser.tgId,
            `💳 <b>Реквизиты для оплаты заказа на ${dirOrder.amount} R$:</b>\n\n` +
            `<code>${text}</code>\n\n` +
        ...
        try {
          await vkSend(payUser.vkId,
            `💳 Реквизиты для оплаты заказа на ${dirOrder.amount} R$:\n\n` +
            `${text}\n\n` +
            `Переведи деньги и пришли скриншот подтверждения сюда (фотографией, не файлом) 👇`
          );
        } catch { }
    ...
    if (!isAdmin && dirFlow) {
      ...
      if (dirFlow.step === "nick_input") {
        ...
        // На экране «чего не хватает» (аккаунт уже найден) принимаем и голый
        // Pass ID — как кнопка «🔢 Он у меня уже есть» в коридоре ВБ. До поиска
        // голое число остаётся ником: чисто цифровые ники в Roblox бывают.
        const dirPassId = parseGamepassUrl(text) ?? (dirFlow.owned ? parseGamepassRef(text) : null);
        if (dirPassId) {
...
/** Клавиатура экрана квеста: `url` рисуется ссылкой, остальное — callback'ом. */
function questKb(screen: QuestScreen) {
  return Markup.inlineKeyboard(
    screen.rows.map((row) =>
      row.map((b) =>
        b.id === "url" && b.url
          ? Markup.button.url(b.label, b.url)
          : Markup.button.callback(b.label, b.id),
      ),
    ),
  );
}
...
/**
 * Показать экран квеста. `edit` — «переписать пузырь ‘Ищу…’»: результат обязан
 * появиться в том же видимом сообщении, иначе он уезжает ниже экрана и человек
 * жалуется, что «анимация исчезла, а ответа нет» (edit-in-place, см. ниже).
 */
async function showQuest(
  ...
  if (edit) await edit(screen.text, extra);
  else await ctx.reply(screen.text, extra);
}

/** Пассы аккаунта в том виде, в каком их читает разбор плана. */
function ownedFromSearch(all: { gamepassId: number | string; name: string; robux: number; image?: string }[]): OwnedPass[] {
  return all.map((g) => ({
...
/**
 * Разбор аккаунта → экран результата.
 *
 * Одна точка на все входы (ввод ника, перепроверка, возврат из ветки ключа):
 * разойдясь, они показывали бы разное про один и тот же аккаунт. План кладётся
 * в сессию целиком — «Подтвердить» оформляет ВЕСЬ набор, а не один пасс.
 */
...
async function handleRobloxNickInput(bot: Telegraf, ctx: any, raw: string): Promise<void> {
  ...
  if (outcome.status !== "user_not_found") {
    void noteProbableNick({ nick, source: "nick-search", wbCode: state.wbCode });
  }

  // ── Аккаунта нет на Roblox — почти всегда опечатка ───────────────────────
  if (outcome.status === "user_not_found") {
    await showQuest(ctx, questNoAccountScreen({ nick, wbCode: state.wbCode }), showResult);
    return;
  }

  // ── Дальше решает разбор плана, а не число найденных пассов ──────────────
  // Раньше здесь было пять текстовых веток («нет пассов», «ни один не за N»,
  // «ровно один», «несколько»), и каждая просила ОДИН пасс ровно за
  // `ceil(номинал/0.7)`. Сайт с 06.09.2026 отвечает иначе: смотрит, что уже
  // выставлено, складывает из этого заказ и просит создать только недостающее.
 ...
 *
 * План может состоять из нескольких частей (заказ на 2000 = 1500 + 500, или
 * размен по тому, что уже выставлено). Первая часть едет в `gamepassUrl`
 * заказа — на неё смотрит вся старая машинерия, — а весь набор ложится в
 * `WbOrderGamepass`, откуда его читают выкуп и обе админки.
 */
async function handleQuestConfirm(bot: Telegraf, ctx: any): Promise<void> {
  ...
  const state: LinkState = { wbCode: quest.wbCode, denomination: quest.denomination };
  pendingLink.set(ctx.from.id, state);
  await processGamepassSubmission(bot, ctx, state, parts[0].gamepassId, { parts });
}

/**
 * Ветка ключа: покупатель прислал Open Cloud ключ.
 *
 * Сообщение с ключом удаляется СРАЗУ — в личном чате бот это умеет, и ключ не
 * остаётся висеть в переписке. Дальше ключ живёт только в памяти этого вызова
 * и в шифрованном хранилище; в лог он не попадает ни при одном исходе.
 */
async function handleApiKeyInput(bot: Telegraf, ctx: any, raw: string): Promise<void> {
  ...
  if (targets.length === 0) {
    pendingApiKey.delete(ctx.from.id);
    await ctx.reply(
      "Создавать больше нечего — проверим аккаунт заново.",
      Markup.inlineKeyboard([[Markup.button.callback("🔄 Проверить ещё раз", CB.findGpRetry)]]),
    );
    return;
  }
  ...
  const edit = async (text: string, extra: Record<string, unknown>) => {
    try {
      await bot.telegram.editMessageText(ctx.chat.id, working.message_id, undefined, text, extra as any);
    } catch {
      await ctx.reply(text, extra);
    }
  };
  ...
  if (outcome.created.length > 0) {
    ...
    // админ должен видеть, что именно он выкупает, даже если дальше что-то
    // упадёт. Ошибка следа не превращает созданный пасс в отказ.
    ...
    }).catch((err) => console.warn("[TG/key] след не записан:", err?.message ?? err));
  }
...
async function handleDirectApiKeyInput(bot: Telegraf, ctx: any, raw: string): Promise<void> {
  ...
  const working = await ctx.reply(questKeyWorkingText(pending.targets), { parse_mode: "HTML" });
  ...
  const outcome = await createPassesByKey({ apiKey: key, nick: pending.nick, targets: pending.targets, game });
  ...
  if (outcome.created.length > 0) {
    ...
    // Созданные пассы достраивают план (набор 1500 + 500 под 2000 и т. п.).
    await finishDirectKey(ctx, flow, pending.nick, outcome, { chatId: ctx.chat.id, messageId: working.message_id });
    return;
  }
...
async function processGamepassSubmission(
  ...
  opts: { viaManualLink?: boolean; parts?: PlanPart[] } = {},
    ...
    // Заказ может закрываться НЕСКОЛЬКИМИ пассами (номинал 2000 = 1500 + 500,
    // либо размен по тому, что у покупателя уже выставлено). Тогда цена
    // основного пасса сверяется с номиналом ЕГО части, а не всего заказа:
    // прайс-гард заказа к разбитому заказу неприменим по построению — ровно
    // так же считает и сайт (`/api/wb-code/select-gamepass`).
    const parts = opts.parts && opts.parts.length > 1 ? opts.parts : null;
    const expectedPrice = parts ? parts[0].price : Math.ceil(state.denomination / 0.7);
    ...
    if (!gamepassInfo.validationSkipped) {
      ...
      // Early nick capture: Roblox already told us the creator — note it
      // (в заметку заказа) even if validation fails below (wrong price / not
      // for sale), so the manager sees the probable nick immediately.
      if (gamepassInfo.creatorName) {
        void noteProbableNick({ nick: gamepassInfo.creatorName, source: "gp-validation", wbCode: state.wbCode });
      }
      ...
      await showResult(
        ...
        `Убедись, что цена геймпасса установлена ровно <b>${expectedPrice} R$</b>. ` +
      ...
      const alertText =
        ...
        `Pass ID: <code>${passId}</code> · Ожидаемая цена: ${expectedPrice} R$`;
      for (const adminId of ADMIN_IDS) {
    ...
    // Набор: выше проверен только головной пасс. Остальные части — тем же общим
    // правилом, что у гейта ВБ на сайте: в продаже, цена = номиналу части,
    // и все пассы одного владельца (робуксы уходят владельцу КАЖДОГО пасса).
    if (parts) {
      const setCheck = await acceptDirectSelection({
        totalAmount: state.denomination,
        gamepassId: passId,
        parts,
        nick: validatedCreator ?? "",
      });
      if (!setCheck.ok) {
        await showResult(`⚠️ ${escapeHtml(setCheck.message)}`, {
          parse_mode: "HTML",
          ...Markup.inlineKeyboard([
            [Markup.button.url("📖 ИНСТРУКЦИЯ", `https://robloxbank.ru/guide?source=wb&skip=1&code=${state.wbCode}`)],
            [Markup.button.callback("🔎 Проверить заново", CB.findGpRetry)],
          ]),
        });
        return;
      }
    ...
    try {
      const txResult = await (db as any).$transaction(async (tx: any) => {
        ...
        if (existingOrder) {
          ...
          if (isProcessing && existingOrder.gamepassUrl === cleanLink) {
            ...
            // «двойные карточки»: site one-tap + bot submit of the same pass).
            return { order: existingOrder, duplicate: true, replacedUrl: null };
          }
          // A DIFFERENT pass on a queued order = замена — the card gets a
          // 🔁 marker so the manager knows it's not a new order.
        ...
        // того же кода не оставило хвост от прошлой попытки. Инвариант «сумма
        // частей равна номиналу» держит `planFromOwned`; проверяем ещё раз тут,
        // потому что разошедшаяся сумма — это не та сумма робуксов, за которую
        // человек заплатил.
        if (parts) {
          const sum = parts.reduce((acc, part) => acc + part.amount, 0);
          if (sum !== state.denomination) {
            throw Object.assign(new Error(`Разбивка не сходится: ${sum} ≠ ${state.denomination}`), { badSplit: true });
          }
          ...
          // Пасс заменили на ОДИН (эта ветка — только когда пасс другой:
          // тот же уже вернулся выше как duplicate). Прежний набор частей
          // относился к прежнему пассу, и оставить его — значит показать
          // менеджеру разбивку, которой у этого заказа больше нет.
          await tx.wbOrderGamepass.deleteMany({ where: { orderId: newOrder.id } });
        }
      ...
      if (err.badSplit) {
        console.error("[TG] Разбивка не сошлась:", err.message);
        await showResult(
          "⚠️ Не сошлась разбивка заказа — проверим руками, чтобы ты получил ровно свою сумму.",
          { parse_mode: "HTML", ...withSupportKb("💬 Написать нам", "split_mismatch", ctx) },
        );
        return;
      }
      if (err.code === "P2002") {
    ...
    await showResult(
      ...
      `\n\nКод ВБ: <code>${state.wbCode}</code> · Статус и бонусы — в меню 👇`,
      {
        ...
        ...Markup.inlineKeyboard([
          [Markup.button.callback("📊 Проверить статус", CB.refreshStatus)],
          [Markup.button.callback("👤 Открыть моё меню", CB.buyerMenu)],
        ]),
      }
    );
...
async function renderOrderCard(order: any, creatorName?: string, isAgeRestricted?: boolean, replacedGamepassUrl?: string, viaManualLink?: boolean) {
  ...
  // Одной волной: код, набор частей и признак «пасс создан по ключу». Каждый
  // заход в базу стоит ~200 мс с прод-хоста, а зависимостей между этими тремя
  // нет — последовательные await превратили бы карточку в полсекунды ожидания.
  const [wbCode, splitRows, autoCreated] = await Promise.all([
    (db as any).wbCode.findUnique({ where: { code: order.wbCode } }),
    (db as any).wbOrderGamepass.findMany({
      where: { orderId: order.id },
      orderBy: { position: "asc" },
      select: { gamepassId: true, amount: true },
    }).catch(() => []),
    // Признак берём из СОБЫТИЙ заказа, а не со слов клиента: маркер 🔑 говорит,
    // что цену и «в продаже» выставили мы, и отвечаем за них тоже мы.
    (db as any).orderEvent.count({
  ...
  const replacedLine = replacedGamepassUrl
    ? (() => {
        const m = replacedGamepassUrl.match(/game-pass(?:es)?\/(\d+)/);
        return `🔁 <b>ЗАМЕНА ГЕЙМПАССА</b>${m ? ` (было: <code>${m[1]}</code>)` : ""}\n`;
      })()
    : "";

  const viaKeyLine = autoCreated > 0
  ...
  // повтор одного пасса с того же аккаунта Roblox вернёт AlreadyOwned.
  const splitLines: string[] = splitRows.length > 1
    ? [
        `🧩 <b>РАЗБИВКА: ${splitRows.length} ${splitRows.length < 5 ? "части" : "частей"}</b> — каждую покупать с ОТДЕЛЬНОГО донора`,
        ...splitRows.map((part: { gamepassId: string; amount: number }, index: number) =>
  ...
  const text = formatAdminNotice({
    ...
    lines: [
      orderRef({ code: order.wbCode, denomination: order.amount }, [
        splitRows.length > 1 ? `${splitRows.length} пасса на ${passPrice} R$ суммарно` : `геймпасс ${passPrice} R$`,
      ]),
    ...
    next: isAgeRestricted ? "игра 18+ — выкупать только вручную"
      : waitingBuyout
        ? (splitRows.length > 1
            ...
            : "купить геймпасс в доноре и нажать «ВЫКУПЛЕНО»")
      : null,
  });
...
async function handleWbCodeTextEntry(bot: Telegraf, ctx: any, tgId: string, text: string): Promise<void> {
  ...
  await ctx.reply(
    ...
    `🔎 <b>Начнём с ника Roblox.</b> Посмотрю, что у тебя уже выставлено: часто заказ собирается из готового и создавать ничего не нужно. Если нужно — покажу, что именно, и дам выбрать способ (сделать самому, прислать номер готового или <b>сделать за тебя</b>).\n\n` +
    `📖 Та же проверка и инструкция со скринами — на сайте: ${textGuideUrl}\n\n` +
    `🔔 А здесь, в боте, придут <b>уведомления о заказе</b> — приняли → выкупаем → готово.`,
    {
      parse_mode: "HTML",
      link_preview_options: { is_disabled: true },
      ...Markup.inlineKeyboard([
        [Markup.button.callback("🔎 Проверить мой аккаунт", CB.findGpStart)],
        [Markup.button.url("📖 Открыть инструкцию", textGuideUrl)],
      ]),
    }
  );
}
...
async function performAdminReject(bot: Telegraf, ctx: any, orderId: string, reason: string) {
  ...
  try {
    ...
    if (updated.count === 0) {
      ...
      await ctx.reply(
        current
          ? `⚠️ Заказ <code>${current.wbCode}</code> уже в статусе <b>${current.status}</b> — отклонение не выполнено.`
          : "❌ Заказ не найден.",
        { parse_mode: "HTML" }
      );
      return;
    ...
    await ctx.reply(`❌ <b>Заказ <code>${order.wbCode}</code> отклонён.</b>\nПричина: <i>${displayReason}</i>`, { parse_mode: "HTML" });
...
export function registerCallbacks(bot: Telegraf): void {
  bot.on("callback_query", async (ctx) => {
    ...
    // Тот же маршрут, что на сайте: результат проверки → выбор способа →
    // ветка. Состояние (план и найденные пассы) лежит в `questPlans`.
    if (data === QUEST.confirm) {
      await ctx.answerCbQuery("Оформляем…").catch(() => {});
      pendingApiKey.delete(ctx.from.id);
      await handleQuestConfirm(bot, ctx);
      return;
    }
    ...
    if (data === QUEST.fork) {
      ...
      if (!quest) {
        await ctx.reply(
          "Сначала посмотрим твой аккаунт — пришли ник Roblox.",
          Markup.inlineKeyboard([[Markup.button.callback("🔎 Ввести ник Roblox", CB.findGpStart)]]),
        );
        return;
      }
    ...
    if (data === QUEST.key || data === QUEST.keyRetry) {
      const quest = questPlans.get(ctx.from.id);
      await ctx.answerCbQuery().catch(() => {});
      if (!gamepassAutocreateEnabled()) {
        // Кнопки без метода быть не должно, но клавиатура живёт в чате дольше
        // флага: старое сообщение не имеет права обещать то, чего уже нет.
        await ctx.reply(
          "Этот способ сейчас недоступен — создай геймпасс по инструкции, там всё по шагам.",
          Markup.inlineKeyboard([[Markup.button.callback("↩️ Другой способ", QUEST.fork)]]),
        );
        return;
      }
      ...
      // Следующее текстовое сообщение — ключ. Ник и ссылку в этот момент не
      // ждём: `pendingRobloxNick`/`pendingLink` сняты, иначе длинная строка
      // ключа ушла бы в разбор ника («ник не похож на ник Roblox»).
      pendingRobloxNick.delete(ctx.from.id);
      pendingApiKey.set(ctx.from.id, { wbCode: quest.wbCode, denomination: quest.denomination, nick: quest.nick });
      await showQuest(ctx, questKeyScreen({
    ...
    if (data === CB.findGpStart || data === CB.findGpRetry) {
      ...
      pendingRobloxNick.set(tgIdNum, state);
      pendingApiKey.delete(tgIdNum);
      await ctx.answerCbQuery().catch(() => {});
      // Обещание изменилось вместе с флоу: раньше бот искал пассы РОВНО за
      // `ceil(номинал/0.7)`, теперь смотрит аккаунт целиком и складывает заказ
      // из того, что уже выставлено (то же самое делает инструкция на сайте).
      await ctx.reply(
        `🔎 Введи свой <b>ник в Roblox</b> (то, как ты заходишь в игру).\n\n` +
        `Посмотрю, что у тебя уже выставлено, и соберу заказ на <b>${order.amount} R$</b> из этого — создавать что-то новое нужно не всегда.\n` +
        `<i>Если под рукой есть ссылка на геймпасс — пришли её, этого тоже достаточно.</i>`,
        { parse_mode: "HTML" }
      );
      return;
    }
    ...
    if (data === CB.sendGpLink) {
      ...
      // На разбитом заказе цена НЕДОСТАЮЩЕГО пасса — не цена всего заказа.
      // Спрашивать «ровно 2858 R$» у человека, которому нужен пасс на 715, —
      // прямой путь к пассу не той цены и к отказу прайс-гарда.
      const linkQuest = questPlans.get(tgIdNum);
      const linkTargets = linkQuest && linkQuest.wbCode === working.order.wbCode
        ? targetsToCreate(linkQuest.plan)
        : [];
      const linkPassPrice = linkTargets.length > 0
        ? linkTargets[0].price
        : Math.ceil(working.order.amount / 0.7);
      await ctx.answerCbQuery().catch(() => {});
      await ctx.reply(
    ...
    if (data.startsWith("sup:")) {
      ...
      await ctx.reply(
        "💬 Поддержка — это <b>живой человек</b> в отдельном чате <b>@RobloxBank_PA</b> (это не бот).\n\nНапиши туда сам — поможем с любым вопросом 👇",
        {
          parse_mode: "HTML",
    ...
    if (data.startsWith("ps:")) {
      ...
      try {
        ...
        const lines: string[] = [];
        lines.push(`📋 <b>Скрипт выкупа</b> · Заказ <code>${order.wbCode}</code>`);
      ...
      try {
        ...
        if (result.success) {
          ...
          const editedText =
            ...
            `Заказ <code>${order.wbCode}</code> · ${info.priceInRobux} R$` +
            priceWarning;
    ...
    if (data.startsWith("admin_reject_init:")) {
      ...
      await ctx.reply(
        `⚠️ Отклонить заказ <code>${rejCode ?? "?"}</code>?\n\nЭто действие уведомит пользователя.`,
        {
          parse_mode: "HTML",
          ...Markup.inlineKeyboard([[
    ...
    if (data.startsWith("confirm_reject:")) {
      ...
      try { await ctx.editMessageText(
        `📋 Выбери причину отклонения заказа <code>${rejCode2 ?? "?"}</code>:`,
        {
          parse_mode: "HTML",
          ...Markup.inlineKeyboard([
    ...
    if (data.startsWith("dgp:")) {
      ...
      const flow = pendingDirectFlow.get(ctx.from.id);
      // Экран «чего не хватает» держит шаг `nick_input` (ждёт Pass ID текстом),
      // но и оттуда можно выбрать пасс под пересчёт.
      if (!flow || !flow.robloxUsername || (flow.step !== "gamepass" && flow.step !== "nick_input")) {
        await ctx.answerCbQuery("Начни заново").catch(() => {});
        return;
      }
    ...
    if (data === CB.directKey) {
      ...
      await startDirectKey(ctx, flow);
      return;
    }

    /* dir_key_saved: ключ уже привязан (кабинет или прошлый заказ) — создаём
       недостающие пассы сами, без единого действия покупателя. Та же дверь,
       что «✨ Создать за меня» в квесте ВБ. */
    if (data === CB.directKeyStored) {
      const flow = pendingDirectFlow.get(ctx.from.id);
      await ctx.answerCbQuery().catch(() => {});
      if (!flow?.robloxUsername || !flow.totalAmount) {
        await ctx.reply("Начни заново — пришли ник Roblox.", Markup.inlineKeyboard([
          [Markup.button.callback("◀️ Назад", CB.directBack)],
        ]));
        return;
      ...
      const nick = flow.robloxUsername;
      const userId = gamepassAutocreateEnabled() ? await dbUserIdFor(ctx) : null;
      const stored = userId ? await loadRobloxApiKeyForUser(keyStore, userId, nick) : null;
      if (!stored) {
        // Ключ отвязали или он принадлежит другому нику — обычная ветка ключа.
        await startDirectKey(ctx, flow);
        return;
      }
      const targets = directKeyTargets(flow.totalAmount).map((t) => t.price);
      const working = await ctx.reply(questKeyWorkingText(targets), { parse_mode: "HTML" });
      const outcome = await createPassesByKey({ apiKey: stored.key, nick, targets });
      if (outcome.created.length > 0) {
        await rememberRobloxApiKey(keyStore, {
          key: stored.key,
          robloxUsername: nick,
          userId,
          orderId: null,
      ...
      await finishDirectKey(ctx, flow, nick, outcome, ctx.chat ? { chatId: ctx.chat.id, messageId: working.message_id } : undefined);
      return;
    }
    ...
    if (data === CB.directSubmit) {
      ...
      // Последняя проверка — общее правило приёма (как гейт ВБ и касса сайта):
      // каждый пасс в продаже, цена части = её номиналу, набор одного владельца.
      const accepted = await acceptDirectSelection({
      ...
      if (!accepted.ok) {
        await ctx.answerCbQuery("Геймпасс изменился").catch(() => {});
        await ctx.reply(`⚠️ ${escapeHtml(accepted.message)}`, {
          parse_mode: "HTML",
          ...Markup.inlineKeyboard([
            [Markup.button.callback("🔄 Проверить заново", CB.directBack)],
            [Markup.button.callback("❌ Отменить", CB.directCancel)],
          ]),
        });
        return;
      }
      pendingDirectFlow.delete(ctx.from.id);
      const recipient = accepted.recipient ?? flow.robloxUsername;
      ...
      try {
        intent = await (db as any).directIntent.create({
          data: {
            ...
            robloxUsername: recipient,
            gamepassId:    flow.gamepassId!,
            gamepassUrl:   flow.gamepassUrl!,
            parts:         intentPartsJson(flow.parts),
            platform:      "TG",
          },
        });
      } catch (err) {
      ...
      const passLine = flow.parts && flow.parts.length > 1
        ? `🧩 Набор: <b>${flow.parts.map((p) => p.amount).join(" + ")} R$</b> (${flow.parts.map((p) => `${p.price} R$`).join(", ")}) — каждая часть сверена`
        : `🎫 Пасс: <b>${flow.gamepassRobux ?? "?"} R$</b>` + (gpOk ? ` (норма)` : ` ⚠️ <b>ожидалось ${flow.passPrice} R$</b>`);
      await Promise.allSettled(ADMIN_IDS.map((id) => tgSend(
        id,
        `🔷 <b>Новая заявка · клиент выбирает оплату</b>\n` +
        `Канал: TG · Ник: <b>${escapeHtml(recipient)}</b> · ${flow.totalAmount} R$ · ${fmtRub(flow.rublePrice!)}\n` +
        passLine +
        (accepted.ownerSwitchedFrom ? `\n⚠️ Назван ${escapeHtml(accepted.ownerSwitchedFrom)}, пасс принадлежит ${escapeHtml(recipient)} — робуксы владельцу пасса` : ""),
      )));
      ...
      const intentMsg =
        ...
        `📦 ${flow.totalAmount} R$ → <b>${escapeHtml(recipient)}</b>\n` +
    ...
    if (data.startsWith("spd:")) {
      ...
      await ctx.reply(
        ...
        `Заказ <code>${spdOrder.wbCode}</code> · ${spdOrder.amount} R$\n\n` +
      ...
      if (sqrUser?.tgId) {
        try {
          await bot.telegram.sendPhoto(
            ...
            {
              caption:
                `💳 <b>Оплата заказа на ${sqrOrder.amount} R$</b>\n\n` +
        ...
        delivered = await vkSendPhoto(
          sqrUser.vkId,
          qr,
          `💳 Оплата заказа на ${sqrOrder.amount} R$\n\n` +
    ...
    if (data.startsWith("cdo:")) {
      ...
      if (cdoUser?.tgId) {
        try {
          await bot.telegram.sendMessage(
            cdoUser.tgId,
            `❌ <b>Заказ на ${cdoOrder.amount} R$ отменён.</b>\n\nЕсли хочешь — создай новый заказ.`,
    ...
    if (data.startsWith("sqi:")) {
      ...
      try {
        newOrder = await (db as any).$transaction(async (tx: any) => {
          ...
          // Набор заявки — в `WbOrderGamepass`, как у коридора ВБ.
      ...
      if (intent.user.tgId) {
        try {
          await bot.telegram.sendPhoto(
            ...
            {
              caption:
                `💳 <b>Оплата заказа на ${intent.totalAmount} R$</b>\n\n` +
    ...
    if (data.startsWith("spi:")) {
      ...
      await ctx.reply(
        `💳 <b>Введи реквизиты для пользователя</b>\nЗаказ <code>${dirCode}</code> · ${intent.totalAmount} R$\n\nПросто напиши — я отправлю напрямую покупателю:`,
        { parse_mode: "HTML" }
      );
    ...
    if (data.startsWith("user_resubmit:")) {
      ...
      await ctx.reply(
        ...
        `<i>Или отправь ссылку / Asset ID.</i>`,
        {
          parse_mode: "HTML",
          ...Markup.inlineKeyboard([
            [Markup.button.callback("🔎 Найти по моему нику Roblox", CB.findGpStart)],
...
async function notifyUserRejected(
  ...
  // Прямой заказ клиент узнаёт по сумме — без внутреннего номера (C2).
