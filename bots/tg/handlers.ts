...
async function notifyUserCompleted(
  ...
  if (isDirectOrder) {
    ...
    tgMsg =
      ...
      `Робуксы появятся в течение 5–7 дней.` +
      pendingLine + `\n\n` +
      `Кстати, для постоянных клиентов у нас есть закрытый формат — без очередей и по лучшему курсу. Пиши напрямую: @RobloxBank_PA\n\n` +
      `Всё ли было удобно? Если есть идеи — напиши, мы читаем каждое сообщение.`;
    vkMsg =
      `✅ Заказ выкуплен! Это уже твой ${completedCount}-й заказ в RobloxBank. Спасибо за доверие! 💛\n\n` +
      `Робуксы появятся в течение 5–7 дней.` +
      pendingLineVk + `\n\n` +
      `Кстати, для постоянных клиентов у нас есть закрытый формат — без очередей и по лучшему курсу. Пиши напрямую: https://t.me/RobloxBank_PA\n\n` +
      `Всё ли было удобно? Если есть идеи — напиши, мы читаем каждое сообщение.`;
  }
