  ...
  for (const user of users) {
    ...
    if (newLevel === 4) {
      msg =
        ...
        `Бонус автоматически добавится к следующей прямой покупке в боте или на robloxbank.ru. Отдельно он не выдаётся и к покупке на WB не применяется.`;
    } else {
      msg =
        `💰 <b>Напоминание: у вас есть бонус ${BONUS_AMOUNT} R$!</b>\n\n` +
        `Бонус за отзыв действует ещё <b>${daysLeft} ${daysWord(daysLeft)}</b> — до ${expiryStr}.\n\n` +
        `Он автоматически добавится к следующей прямой покупке в боте или на robloxbank.ru. Отдельно бонус не выдаётся и к покупке на WB не применяется.`;
    }
