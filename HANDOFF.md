...
## Сессия 2026-06-25 — Новая ценовая модель + бонусы + скидки (⏳ ГОТОВО, НЕ ЗАДЕПЛОЕНО)

> **Статус:** код готов, tsc/esbuild чисто. Миграция создана, НЕ применена к Neon. Деплой НЕ выполнен.
...
1. `npx prisma migrate deploy` — применить миграцию к Neon
2. `npx prisma generate` — обновить клиент
3. `git add . && git commit && git push` — Coolify подхватит Web+Guide+TG+VK

### Что осталось

- [ ] Применить миграцию к Neon
- [ ] Деплой
- [ ] Cron-скрипт пятничного пуша спеццен
