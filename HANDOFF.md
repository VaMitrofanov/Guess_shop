...
## Сессия 2026-06-22 — Обновление WB-инструкции под новый дизайн Roblox Creator

### Контекст
Roblox обновил интерфейс Creator Hub: раздел «Price» переименован в «Sales», поле «Default Price» → «Price», чекбокс «Enable regional pricing» заменён на переключатель «Managed pricing». Скриншоты в инструкции устарели.

### Что сделано (финал)

**Подход:** скриншоты Roblox остаются **чистыми** (никаких «запечённых» рамок/подписей через PIL — это давало кривые выделения). Все выделения, подписи и живая цена реализованы **CSS-слоем поверх** (`.wbi-anno` / `.wbi-box` / `.wbi-tip` / `.wbi-price6`). Проценты позиций выверены пиксель-в-пиксель и проверены реальным рендером через Playwright (`/guide?source=wb&test=1&nom=1000`).

**Ассеты переведены на PNG** (резкость UI-текста), старые `.jpg` шагов 4/5/6 удалены:
- `public/guide/wb-step4-menu.png` — **общий план**: левый рейл приложения (Home/Creations/Store…) + раскрытое ☰-меню Creator Hub. Видно, где находится раздел. Кроп из скрина пользователя `Downloads/Screenshot 2026-06-22 at 00.56.42.png` (0,500,895,1985).
- `public/guide/wb-step5-regional.png` — экран **Sales**: Price + тумблеры. «50» в поле стёрто. Кроп из `Downloads/Screenshot 2026-06-22 at 00.10.13.png` (0,657,1290,1345).
- `public/guide/wb-step6-price.png` — крупный план пустого поля Price. «50» стёрто. Кроп того же скрина (0,846,1290,1158).

**CSS-оверлеи (в `WBInstructionV2.tsx`, блок CSS):**
- Шаг 4: золотая рамка на `Monetization`, зелёная на `Passes`.
- Шаг 5: зелёная рамка + `ВКЛЮЧИ` на `Item for sale`, жёлтая + `ВЫКЛЮЧИ` на `Managed pricing`; живая цена `{price}` в поле (top 57.1%).
- Шаг 6: зелёная рамка на поле + бейдж `СЮДА ВСТАВЬ ЦЕНУ` (с указателем-кареткой) + живая цена `{price}` в поле (top 65.7%).
- `.wbi-price6` использует `cqw` (container-type:inline-size) — масштабируется вместе со скрином, шрифт под Roblox.

**Чтобы перегенерить ассеты:** PIL-скрипты разовые; кропы брались из двух исходников в `~/Downloads`. Если меняется UI Roblox — заменить исходники, повторить кропы (координаты выше), проверить рендером Playwright. Проценты рамок задаются инлайн-стилями в JSX (легко двигать).

### Файлы изменены
- `src/app/guide/WBInstructionV2.tsx` — figure шагов 4/5/6 → `.wbi-anno` + CSS-оверлеи; новый CSS-блок (`.wbi-box/.wbi-tip/.wbi-price6`); удалены `.wbi-priceshot/.wbi-liveprice`
- `public/guide/wb-step4-menu.png` / `wb-step5-regional.png` / `wb-step6-price.png` — **новые** чистые PNG
- Удалены: `wb-step{4-menu,5-regional,6-price}.jpg` (заменены PNG)

### Не тронуто
- `GuideClient.tsx` — старый (не-WB) гайд по-прежнему ссылается на «Default Price» и «Enable regional pricing» в анимированном мини-UI. Нужно обновить отдельно, если актуально.
- `bots/tg/handlers.ts`, `bots/vk/handlers.ts` — боты не дублируют текст инструкции, только отправляют ссылку на неё. Менять не нужно.
- Старые ассеты `wb-step4-createpass.mp4`, `wb-step4-poster.jpg`, `wb-step5-price.jpg` — остались в `/public/guide/`, можно удалить.

### Деплой
Не задеплоено. Для деплоя — коммит + push в main → Coolify auto-deploy.

---

## Сессия 2026-06-21 — Привязка геймпасса THRC7TM + обновление Google Sheets

### Что сделано

#### 1. Привязка геймпасса к заказу THRC7TM
Скрипт: `scripts/link-gamepass-order.mjs`
- wbCode = `THRC7TM`, gamepassUrl = `https://www.roblox.com/games/127926411525036/poop223383s-Place`
- Статус обновлён `AWAITING_GAMEPASS → PENDING`
- Карточка отправлена 3 админам (TG), пользователь (VK) — уведомление не доставлено (нет разрешения на сообщения)
- Запускается: `node scripts/link-gamepass-order.mjs <WB_CODE> "<URL>"`

#### 2. Обновление Google Sheets (таблица заказов)
**Таблица:** `https://docs.google.com/spreadsheets/d/1IYehE2cuV67gf2efOXFHaGqWXBljfsSHt0YAlkRE81Q`

**Структура таблицы:**
- Каждая вкладка = отдельная дата: `18.06`, `19.06`, `20.06`, ...
- Колонки (A–I): площадка | номинал | ссылка | ник | статус | грязных | код WB | прямой | дата
- Вкладка `БД Export` — консолидированный экспорт (строки 111–116 = заказы 18.06, которые были там изначально)

**Как обновлять через Google Apps Script:**
1. Открыть таблицу → Расширения → Apps Script
2. Вставить скрипт (последняя версия в `/tmp/wb_dates.js`)
3. Запустить функцию `fixDateSheets`

**Важные уроки:**
- `getSheets()[0]` — НЕ использовать, берёт первую вкладку (по имени "10.06.2026"), а не нужную
- Нужный лист: `ss.getSheetByName("18.06")` / `"19.06"` / `"20.06"` — по дате заказа
- Писать в каждый лист по дате, не в `БД Export`
- Кириллицу в строках — кодировать через `\uXXXX` (unicode-escape), иначе при вставке через браузер кавычки ломаются и Apps Script выдаёт SyntaxError на строке с кириллицей
- `existingCodes` фильтрует дубли: любая 7-значная строка вида `[A-Z0-9]{7}` в столбце G считается существующим кодом
- Скрипт идемпотентен: повторный запуск не дублирует строки (проверяет по коду ВБ в col G)

**Статусы в таблице:**
- `на выполнении` — PENDING (геймпасс привязан, ждёт выкупа)
- `в работе` — IN_PROGRESS (выкупается прямо сейчас)
- `готово` — COMPLETED
- `ожидание ГП` — AWAITING_GAMEPASS (геймпасс ещё не прислал)
- `отклонён` — REJECTED

**Цвета статусов:** зелёный=готово, синий=в работе, жёлтый=на выполнении, голубой=ожидание ГП, красный=отклонён

---

## Сессия 2026-06-19 — Массовая рассылка: объявление об обновлении геймпассов Roblox

### Контекст
Roblox обновил систему безопасности: прямые ссылки на геймпассы перестали работать (404), «пустые» плейсы получают Unavailable на вкладке Store. Нужно было оповестить всех пользователей с незавершёнными заказами.

### Что сделано

#### 1. Проверка инструкции в ботах
Подтверждено: кнопка «📖 ИНСТРУКЦИЯ» во всех ботах генерирует **персональную ссылку** с `skip=1&code=XXX` для пользователей с активным заказом (статус `AWAITING_GAMEPASS`). Код не нужно вводить повторно. Это работает в:
- `buildStatusMessage()` (TG, `handlers.ts:597-600`)
- Session recovery (TG `handlers.ts:827`, VK `handlers.ts:537-538, 569-571`)
- `handleRefActivation` (VK `handlers.ts:837`)

#### 2. Рассылка объявления
Скрипт: `scripts/broadcast-announcement.mjs` + `scripts/broadcast-tg-only.mjs`
Фото: `scripts/broadcast-photos/` (3 скриншота настроек Roblox Creator)

**Результаты (2026-06-19):**

| Канал | Действие | Результат |
|-------|----------|-----------|
| TG канал `@Roblox_Bank_Tg` | Текст + 3 фото | ✅ msg_ids: 9, 10, 11, 12 |
| TG пользователи (34) | Forward из канала | ✅ 33 доставлено, 1 заблокировал бота |
| VK стена `bankroblox` | Текст (без фото) | ✅ post_id: 11. Фото не прикрепились — `photos.getWallUploadServer` недоступен для community-токена (нужен user-токен). |
| VK пользователи (28) | Текст + 3 фото в ЛС | ✅ 23 доставлено, 5 не давали разрешение на сообщения |

**Итого: 56/62 (90%) получили объявление.**

Не получили:
- TG `5778547870` — бот заблокирован пользователем
- VK 5 юзеров — не давали разрешение боту на сообщения (`Can't send messages for users without permission`)

#### 3. Env-обновление
- `.env` обновлён: `TG_TOKEN` заменён на production-токен (`8688639442:AAG...`), добавлены `TG_CHANNEL_ID`, `VK_TOKEN`, `VK_GROUP_ID` для будущих скриптов.
- Старый локальный TG_TOKEN (`8615458497:AAE...`) был dev/другой — заменён на production.

### Файлы добавлены
- `scripts/broadcast-announcement.mjs` — полный скрипт рассылки (TG + VK)
- `scripts/broadcast-tg-only.mjs` — TG-only (повторный запуск после фикса токена)
- `scripts/broadcast-photos/` — 3 скриншота из Roblox Creator (01-questionnaire, 02-public-place, 03-content-settings)

### Ограничения VK wall-постов с фото
Community-токен (VK_TOKEN) **не может** загружать фото на стену группы (`photos.getWallUploadServer` → error 27 "method is unavailable with group auth"). Для постов с фото нужен **user-токен** с правами `wall,photos`. Фото в ЛС работают (`photos.getMessagesUploadServer` + `photos.saveMessagesPhoto` — ок).

---

## Сессия 2026-06-17 — Редизайн ботов TG + VK: инструкция-first + фикс закрытых геймпассов

**Коммит:** `ee8d732` → `main` → авто-деплой обоих ботов через Coolify.
**Деплой:** TG_bot (`lyz78enntugna9em1biopinr`, SG) ✅ finished | VK_bot (`gmtpfqosgoz23vjyxyczuic9`, RF) ✅ finished.

### Что изменено

#### 1. Боты: «инструкция на первом месте» (TG: `bots/tg/handlers.ts`, VK: `bots/vk/handlers.ts`)
Раньше бот сам пытался объяснить создание геймпасса (длинные сообщения, развилки, дубли с сайтом). Теперь:
- **Активация кода** (с сайта через ref или текстом в чат): «✅ Код активирован! ⚠️ Без выполнения инструкции робуксы выкупить невозможно» + крупная ссылка-кнопка `📖 ИНСТРУКЦИЯ` + `🔎 Ввести ник Roblox`.
- **Приветствие нового пользователя** (без кода): ссылка на инструкцию + «Есть код? Напиши сюда / Геймпасс готов? Напиши ник».
- **Восстановление сессии** (бот перезапустился / пользователь вернулся): «Код активирован, если геймпасс не создан — инструкция, когда готов — напиши ник».
- **Все ветки поиска по нику** (no_gamepasses, wrong_price, ошибка API): отправляют к инструкции + кнопки «Уже сделал — проверить» и «Помощь».
- Убраны `bonusText`, `isGuideMode`-развилки, длинные inline-инструкции.
- Тон TG и VK — идентичный.

#### 2. Фикс закрытых геймпассов (`bots/shared/roblox.ts`)
- **Фильтр `isForSale`**: `!== false` → `=== true` (строгий). Раньше геймпассы без поля `isForSale` (API не всегда возвращает) считались продаваемыми — теперь нет. Строка ~808.
- **30-дневный порог для приватных игр** убран: раньше блокировались только «свежие» геймпассы в закрытых плейсах (roproxy fallback, строки ~496-508). Теперь ЛЮБОЙ геймпасс в приватной игре блокируется, если основные эндпоинты его не подтвердили.
- Комментарий к фильтру обновлён (строки ~636-637).

### Воркфлоу клиента (полная карта — тупиков нет)

```
[С САЙТА]                           [НАПРЯМУЮ В БОТ]
robloxbank.ru → код → ref в бот     «Начать» / любое сообщение
         ↓                                   ↓
   «✅ Код активирован!              «👋 Помогу обменять WB-карту»
    ⚠️ Инструкция обязательна»       «📖 Инструкция» + «Код? Напиши»
    [📖 ИНСТРУКЦИЯ] [🔎 Ник]               ↓ (вводит код)
         ↓                           → то же что «С сайта»
   Пишет ник Roblox
         ↓
   🔎 Поиск геймпассов
     ├─ Ник не найден → «Опечатка?» [🔎 Ещё раз] [💬 Помощь]
     ├─ Нет публичных GP → «Не создан / закрыт» [📖 ИНСТРУКЦИЯ] [🔎 Проверить]
     ├─ Неверная цена → «Нужен на XXX R$» [📖 ИНСТРУКЦИЯ] [🔎 Проверить]
     └─ Нашёл → фото + «Это он?» [✅ Да, выкупаем] → Заказ → Админ
```

**Каждая ветка** имеет выход: кнопка «Попробовать ещё раз», ссылка на инструкцию, или «Помощь» → @RobloxBank_PA.
**Подписка на канал** (TG/VK) — мягкий gate: блокирует до подписки, потом продолжает автоматически.

### Файлы изменены
- `bots/tg/handlers.ts` — все user-facing сообщения (registerStart, handleWbCodeTextEntry, buildStatusMessage, handleRobloxNickInput, session recovery)
- `bots/vk/handlers.ts` — все user-facing сообщения (handleRefActivation, «Начать», handleIdleMessage, nick search branches)
- `bots/shared/roblox.ts` — фильтр `isForSale` + убран 30-дневный порог приватных игр

---

## Сессия 2026-06-16 — ИТОГИ ДНЯ (WB-инструкция в проде) + ПЛАН НА ЗАВТРА (ВЫПОЛНЕН ↑)
...
## ✅ ПЛАН НА 2026-06-17 — Редизайн ботов (TG + VK) — ВЫПОЛНЕН

**Выполнено в сессии 2026-06-17 выше.** Коммит `ee8d732`, оба бота задеплоены.

Решения по открытым вопросам:
- Ввод по ссылке/Asset ID **оставлен** (handleGamepassLink всё ещё работает), но **основной путь — ник**.
- Код с WB-карты — **обязателен** (и с сайта, и текстом в бот).
- Прямые заказы (не-WB) — **не тронуты**, работают как прежде.
- Единый тон TG и VK — **сделан**.

**Важно:** бот-поверхность НЕ покрыта тестами — менять аккуратно. Деплой ботов — авто по push в `main` (watch-paths: `bots/`). TG на SG `5.223.95.11`, VK на RF `89.110.94.117`.
