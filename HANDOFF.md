...
## Сессия 2026-06-16 — ИТОГИ ДНЯ (WB-инструкция в проде) + ПЛАН НА ЗАВТРА

**Среда:** из неё ЕСТЬ сеть к GitHub/Coolify/Neon и SSH `root@89.110.94.117` (git push, Coolify API, Prisma→Neon, ssh/docker — всё работает отсюда). Прежние заметки «нет сети» — устарели.

### ✅ Что в проде (финал дня, guide-контейнер, образ `ecb325c`)
WB-редизайн `WBInstructionV2` живёт. Сегодняшние правки (все в `main`, последний коммит `ecb325c`):
- **testdev**: `?source=wb&test=1` / `code=TESTDEV` → инструкция напрямую, кнопки TG/VK **инертны** (без бота и админ-карточки). Обычный WB-путь не тронут.
- **preview**: `?source=wb&preview=1` → инструкция напрямую, **как будто код активирован**, с рабочими кнопками.
- **Шаг 1** (текст): «официальный сайт Roblox — где играешь и покупаешь робуксы; кнопка ведёт прямо в нужный раздел» + мелким «открой её в браузере, где вошёл в аккаунт Roblox».
- **Шаг 6** (цена): убрана формула `÷0.7 · 30%`; «Номинал твоей карты: [N]» + крупная копируемая цена + «выстави эту цену, чтобы на руки пришло N R$».
- **CTA-кнопки**: крупные одностроч. **«Telegram» / «ВКонтакте»**, белые иконки, симметричные (раньше «Получить в Telegram» переносился, VK-лейбл был крошечный с синей иконкой). VK увеличен scoped-стилем `.wbi-vkwrap` — глобальный `VKAuthButton` НЕ тронут.

### 🔑 ПОСТОЯННЫЕ ССЫЛКИ (только апекс `robloxbank.ru`, НЕ `www.`!)
- Инструкция с рабочими кнопками: **`https://robloxbank.ru/guide?source=wb&preview=1`** (опц. `&nom=1000`)
- Инструкция без перехода в боты (кнопки инертны): **`https://robloxbank.ru/guide?source=wb&test=1`**
- `source=wb` в URL **обязателен** — это требование роутинга Traefik (см. ниже).
- Реализация preview: проп `previewMode` (`page.tsx`→`GuideClient`), аддитивно `directInstruction = testMode || previewMode`; обычный путь и `test=1` не сломаны.

### 🐛 КЛЮЧЕВАЯ РАЗГАДКА: `/guide` на `www.` и апексе обслуживают РАЗНЫЕ контейнеры
Полдня деплои «не виделись», потому что проверялось через `www.robloxbank.ru`. Фактические Traefik-лейблы (Coolify генерит):
- **guide-контейнер** (`ebac6…`): `guide-app` = `Host(`robloxbank.ru`) && Path(`/guide`) && Query(`source`,`wb`)` → **только апекс**.
- **main-контейнер** (`robloxbank-web`, `z10ws7m1…`): `main-web` = `Host(robloxbank.ru) || Host(`www.robloxbank.ru`)`.

→ `www.../guide?source=wb` НЕ матчит guide-правило (нет www) → уходит на **main**. `served-by=RobloxBank-Guide-v2` есть в `page.tsx` ОБОИХ образов → не различает контейнеры (вводил в заблуждение). **Различать контейнеры по пропсу `previewMode`** в сериализованных пропсах: `curl … | grep -oE '\{[^{}]*"isWB[^{}]*\}'` — есть только в свежем guide-образе.

### Тест-коды
`scripts/reset-test-code.mjs` доработан: сеет/рециклит **4 кода** (`TESTDEV` 500, `TEST300` 300, `TEST500` 500, `TEST700` 700) с **`isTest=true`** (вне статистики). Запущен → все 4 `AVAILABLE`. Рециклить: `npm run dev:reset-test`.

### Деплой guide (важные грабли)
- Авто-деплой по push в `main` **НЕ подхватывает** `RobloxBank-Guide` — триггерить вручную:
  ```bash
  curl -s -X POST "http://89.110.94.117:8000/api/v1/deploy?uuid=ebac6llpah5n2x58rb64yn8j&force=true" -H "Authorization: Bearer $COOLIFY_TOKEN"
  ```
- guide-контейнер пересоздаётся при force-deploy нормально. **main-контейнер застрял на `bfc4b8a`** (Coolify-квирк «custom internal name → no rolling update») — для реальных юзеров не критично (`bfc4b8a` уже содержит редизайн + testdev-фикс; `preview` им не нужен).

### Остаётся (инфра, НЕ блокирует — апекс работает)
- **Свести www→apex для `/guide`:** добавить `www.robloxbank.ru` в Host(...) guide-роутеров (`guide-app`/`guide-assets`/`guide-api`) через Coolify → Settings → Advanced → Custom Traefik Labels. ⚠️ Routing на живом трафике — аккуратно.
- **Обновить main-контейнер** (recreate): Coolify UI Redeploy, либо `docker rm -f robloxbank-web` + force-deploy.
- Реальные коды БД целы: `AVAILABLE 920 / RESERVED 7 / CLAIMED 73` (до и после правок тест-кодов идентично).
- Реальный рендер в Telegram WebView (haptics/видео/живая цена) отсюда не проверить — глазами на ссылках выше.

---

## 📋 ПЛАН НА ЗАВТРА (2026-06-17) — Редизайн ПОЛЬЗОВАТЕЛЬСКОГО бота (TG + VK)

**Цель (со слов владельца):** переделать пользовательский бот в максимально простой — **приём ника Roblox → выдача/показ инструкции, ничего лишнего.** Детали проговариваем завтра, ниже — рамка.

**Идея:** бот = тонкий мост «ник → выкуп пасса». Главная инструкция теперь живёт на сайте (`/guide`, редизайн готов) — бот не должен дублировать длинные объяснения. Минимум шагов, минимум текста, минимум развилок.

**Что разобрать завтра (вход в задачу):**
- Текущие user-флоу TG: `bots/tg/handlers.ts`; VK: `bots/vk/handlers.ts`; общая логика входа/кодов: `src/auth.ts`, `bots/shared/*`.
- Где сейчас бот «перегружен»: длинные сообщения-инструкции, лишние развилки, дубли с сайтом — кандидаты на вырезание.
- Ядро, которое ОСТАЁТСЯ: приём ника Roblox → поиск/проверка геймпасса (`bots/shared/roblox.ts`) → карточка админу на выкуп → подтверждение + бонус за отзыв.
- Что заменить ссылкой на сайт-инструкцию вместо текста в боте (короткая ссылка на `/guide`).

**Открытые вопросы на завтра (уточнить у владельца):**
- Оставляем ли в боте ввод по ссылке/Asset ID, или **только ник**?
- Точка входа: код с карточки WB по-прежнему обязателен в боте, или бот сразу про ник?
- Что с прямыми заказами (не-WB) — трогаем или только WB-коридор?
- Единый текст/тон для TG и VK (сейчас логика дублируется в двух хендлерах).

**Важно:** бот-поверхность НЕ покрыта тестами и рискованная (см. прежние сессии) — менять аккуратно, по одному флоу, с проверкой в проде. Деплой ботов — отдельный (TG на SG `5.223.95.11`, VK на RF), авто-деплой по push **не** на всё — см. секцию про деплой ботов ниже.

---

## Сессия 2026-06-16 (вечер) — WB-редизайн ЗАДЕПЛОЕН в прод + testdev починен

**Итог: всё в проде и работает.** Из этой среды сеть к GitHub/Coolify/Neon **есть** (старая заметка «нет сети» устарела — `git push`, Coolify API и Prisma→Neon отрабатывают отсюда).

### Что задеплоено
- Коммит `bfc4b8a` (поверх `9d80d62`) → `main` (push) + ветка `wb-instruction-redesign` (push с трекингом).
- **Правка `bfc4b8a`:** `WBInstructionV2` получил явный проп `testMode`. В testdev (`?source=wb&test=1` или `code=TESTDEV`) кнопки Telegram/VK теперь **инертны** (видны для QA, но бот и админ-карточка НЕ дёргаются) + подпись «testdev: кнопки отключены». **Обычный WB-путь (`testMode=false`) — байт-в-байт прежняя разметка, не тронут.**

### Нюанс деплоя (важно на будущее)
- Push в `main` авто-задеплоил только **`RobloxBankWeb`**. Сервис **`RobloxBank-Guide`** (UUID `ebac6llpah5n2x58rb64yn8j`) авто-деплой **НЕ подхватил** — пришлось триггерить вручную:
  ```bash
  curl -s -X POST "http://89.110.94.117:8000/api/v1/deploy?uuid=ebac6llpah5n2x58rb64yn8j&force=true" -H "Authorization: Bearer $COOLIFY_TOKEN"
  ```
  Деплой `gjflm3foad7zbx1qio9jy1tv` → `finished`, собран коммит `bfc4b8a` (подтверждено Coolify API).

### testdev «не работал» — причина и фикс
- URL-режим (`test=1` / `code=TESTDEV`) работает без БД — код есть в бандле. Реальная проблема: **тест-коды `TEST300/TEST500/TEST700` были `CLAIMED`** (израсходованы), а шаг деплоя `npm run dev:reset-test` был пропущен.
- **`scripts/reset-test-code.mjs` доработан:** теперь сидит/рециклит **4 кода** (`TESTDEV` 500, `TEST300` 300, `TEST500` 500, `TEST700` 700) и ставит **`isTest=true`** (раньше не ставил → код попадал бы в статистику).
- Запущен → все 4 теперь `AVAILABLE, isTest=true`.

### Верификация прода (всё ✅)
- Задеплоенный бандл guide содержит `wbi-root` + `TESTDEV` (commit `bfc4b8a`).
- Все 8 медиа-ассетов V2 (`wb-step2/3/4/5/6*` jpg+mp4+постеры) → `200`, верные content-type.
- `GET /api/wb-code?code=…` на проде: `TESTDEV/TEST300/TEST500/TEST700` → `claimed:false` + верный номинал; контроль `ZZZZZZZ` → 404.
- **Реальные коды целы (write через upsert-by-code, без массовых апдейтов):** `AVAILABLE 920 / RESERVED 7 / CLAIMED 73` — идентично до и после обоих запусков скрипта.

### Остаётся (не блокирует)
- Авто-деплой `RobloxBank-Guide` по push в main не срабатывает — проверить watch-paths/webhook сервиса в Coolify (пока триггерим вручную force-deploy).
- Подавление админ-уведомлений для `isTest`-кодов в **ботах** (TG/VK + auth.ts) — при реальном прогоне тест-кода через бот админ всё ещё получит карточку. Site-проход (`test=1`/инертные кнопки) уже без оповещений.
- Реальный рендер в Telegram WebView (haptics/видео/живая цена) отсюда не проверить — финальная глазами на `?source=wb&test=1`.

---

## [АРХИВ] Сессия 2026-06-16 — WB-инструкция: перенос в код (справка по составу компонента)

> **Статус УСТАРЕЛ:** всё ниже уже ЗАДЕПЛОЕНО и доработано — актуальное см. «ИТОГИ ДНЯ» в начале файла. Раздел оставлен как справка по составу `WBInstructionV2` (что внутри, ассеты, роутинг). Игнорируй фразы «НЕ задеплоено / нет сети / нужен push владельцу» — они неактуальны.
...
- **Источник:** 6 PDF «Массовое создание N) новая партия.pdf» в `~/Downloads` (по 50 карт = 300), дубль-копия списком — `~/Desktop/Коды_500.xlsx` (6 столбцов × 50). `/tmp/codes.txt` НЕ durable (очистится) — брать коды из xlsx или PDF.
- **Проверки пройдены (всё сходится):** PDF↔список — 300/300, множества совпадают 1-в-1, порядок PDF1..6 = список 1..300, номинал «500» на всех картах, дублей внутри 0; против БД (1004 кода) — **0 пересечений**, внутри списка дублей 0.
- **Как залить (когда скажут):** создать 300 `WbCode` через Prisma (`denomination=500`, `status=AVAILABLE`, `isUsed=false`, `isTest=false`), `batch`-метка предложена `2026-06-15-500`. Локальный скрипт: `@prisma/client` + `@prisma/adapter-pg` + `pg` Pool с `DATABASE_URL` из `.env` (как в `src/lib/prisma.ts`; миграции shadow-db сломаны — писать напрямую через клиент/`createMany`). ⚠️ Это запись в прод-базу — только после явного подтверждения.
...
## 📷 Прямой заказ: отправка СБП-QR покупателю (2026-06-16, коммит `22ddeaf`)

**Проблема (со слов владельца):** в прямом заказе нельзя было отправить покупателю QR-картинку для оплаты — кнопка слала только текстовые реквизиты; фото-механизма не было; в VK вообще не было отправки фото (`vkSend` — только текст). Плюс баг: админ, приславший фото в режиме «ввод реквизитов», проваливался в ветку отзыва («у тебя нет выполненных заказов»).

**Решение (одной кнопкой, TG + VK):**
- **QR хранится в БД, НЕ в репозитории.** Репо `VaMitrofanov/Guess_shop` **публичный** (Coolify билдит из него) — поэтому картинку в `public/` или `bots/` коммитить нельзя (засветится). QR лежит base64 в `GlobalSettings.sbpQrBase64` (колонка добавлена raw-`ALTER`; залито напрямую через pg, 47012 b64 → 35259 байт JPEG 256×256).
  - **Обновить QR без редеплоя:** `UPDATE "GlobalSettings" SET "sbpQrBase64"='<base64>' WHERE id='global';` + рестарт бота (кэш в памяти процесса).
- `bots/shared/sbp.ts` — `getSbpQrBuffer()` читает QR через `$queryRaw` (не завязан на регенерацию Prisma-клиента), кэширует буфер.
- `bots/shared/notify.ts` — `vkSendPhoto(vkId, Buffer, caption)`: полный VK upload-флоу (`photos.getMessagesUploadServer` → multipart upload → `photos.saveMessagesPhoto` → `messages.send` с `attachment=photo<owner>_<id>`). Node 22: глобальные `fetch`/`FormData`/`Blob`.
- `bots/shared/admin.ts` — на карточке прямого заказа кнопка **`📷 Отправить QR (СБП)`** (`CB.sendQr` → `sqr:`); прежняя «Отправить реквизиты» переименована в «Реквизиты текстом».
- `bots/tg/handlers.ts` — хендлер `sqr:`: тапом шлёт QR + сумму (`amount × DIRECT_RATE`, 0.7) покупателю — TG через `bot.telegram.sendPhoto({source: buffer})`, VK через `vkSendPhoto`. Переводит заказ в `PAYMENT_PENDING`, `paymentDetails="СБП QR"`, вооружает приём скрина/чека (TG: `pendingPaymentScreenshot`; VK: DB-driven по `PAYMENT_PENDING`+`isDirectOrder`, `vk/handlers.ts:679`). **Разрешён повторный пересыл** в `PAYMENT_PENDING` (в отличие от `spd:` — фикс тупика «реквизиты уже отправлены»).
- Фикс бага: гард в начале `bot.on("photo")` — фото от админа в режиме реквизитов теперь не улетает в ветку отзыва, а подсказывает кнопку/текст.

**После QR** покупатель шлёт скрин/чек → карточка админу `[✅ Оплата принята]/[❌]` (как с отзывами ВБ) → `pay_ok` → заказ в `AWAITING_GAMEPASS` + инструкция по геймпассу (TG и VK ветки уже были). Кнопка живёт на **каждой** карточке прямого заказа → переиспользование «другим» работает само.
...
**Важный нюанс с кнопкой:** кнопка появляется только на **новых** карточках (`sendAdminDirectOrderCard` отправляется один раз при создании заказа). Карточки, отправленные админам ДО деплоя, кнопку не получат — Telegram их не обновляет. Если нужно дослать QR по старому заказу: либо перевыпустить карточку, либо отправить вручную (как ниже).

**Рита (заказ `RZ33D5` / `cmqfl3ziz…d5`) — закрыто вручную 2026-06-16:** её карточка была до-деплойная (без кнопки). QR отправлен боевым путём напрямую в VK (тем же `vkSendPhoto`-кодом, VK msg 1724), заказ переведён в `PAYMENT_PENDING`. Ждём её скрин/чек → карточка `[✅/❌]` админам. Локальный `TG_TOKEN` в `.env` протух (401, ротирован) — актуальные токены брать из Coolify env сервисов.
...
## Сессия 2026-06-16 — Редизайн WB-инструкции (макет утверждён, перенос в код в работе)

**Статус:** макет утверждён владельцем («стало лучше»). Зафиксирован в репозитории. Полный перенос в `src/app/guide/GuideClient.tsx` и деплой — после финализации шагов 3/4/5 (см. TODO).

### Что сделано
- Собран отдельный превью-макет нового WB-коридора (7 шагов) на реальных скриншотах/видео из записей экрана владельца. Файл: `docs/wb-instruction-mockup.html` (самодостаточный, медиа вшиты, открывается в браузере).
- Подготовлены и положены боевые ассеты в `public/guide/`:
  - `wb-step2-public.jpg` — карточка игры с зелёным бейджем Public (ник замазан).
  - `wb-step3-menu.jpg` — меню Configure с обведённым Questionnaire (статичный слайд, ник замазан).
  - `wb-step3-answers.mp4` — клип ответов «No» с зелёными галочками (~108 КБ, без звука, луп).
  - `wb-step4-createpass.mp4` — клип пути создания пасса: ☰ → Monetization → Passes → Create Pass (~104 КБ).
  - `wb-step5-price.jpg` — страница Sales с обводками ① Default Price и ② Enable regional pricing (пустая).

### Утверждённая структура (7 шагов)
1. Открой Creator Hub — крупная кнопка-ссылка.
2. Найди свою игру — должна гореть зелёная Public.
3. Сделай игру Public — опрос Questionnaire (все ответы **No** → синяя кнопка дважды). [слайд меню + видео ответов]
4. Создай геймпасс — Monetization → Passes → Create Pass. [видео]
5. Поставь цену — **сначала снять галочку Enable regional pricing**, потом Default Price + Save. [статичный скрин с обводками]
6. Запомни свой ник.
7. Бот всё сделает сам — вводишь ник Roblox, бот находит/проверяет/выкупает пасс + бонус за отзыв; можно ссылку/Asset ID, но по нику проще.

- Блок сверху: **«⚠️ ВАЖНО ВЫПОЛНИТЬ 2 ПУНКТА»** (Place = Public; галочка регионалок снята — иначе геймпасс физически не выкупить).
- Интерактивная карточка «ВАША ЦЕНА ПАСА»: ввод номинала → цена пасса = ceil(номинал/0.7), копирование по клику.
- Кнопки «Получить в Telegram / ВКонтакте» — крупные, объёмные.
- Базовые шрифты увеличены (аудитория — дети/новички, 80% трафика — мобилки/планшеты).

### TODO — допиливаем (НЕ финал)
- [ ] **Шаг 3** — доработать (плавность слайдов/анимации).
- [ ] **Шаг 4** — доработать.
- [ ] **Шаг 5** — ПЕРЕДЕЛАТЬ: разбить на **2 отдельных шага** (сейчас перегружен инфой — снятие галочки регионалок и установка цены развести).
- [ ] Полный перенос редизайна в `src/app/guide/GuideClient.tsx`: повысить `PublicGameBlock` (сейчас под шагом 02) до отдельного крупного шага; заменить старые `public/guide/public-*.jpg` на новые `wb-step*` ассеты; ленивая загрузка медиа (preload="none" + poster + IntersectionObserver, не автоплеить всё сразу); увеличить базовые шрифты.
- [ ] Оптимизация (мобилки, 80% трафика): пауза `setInterval`-анимаций вне экрана (сейчас 11 шт. крутятся постоянно, 0 IntersectionObserver); пережать/уменьшить старые `public/guide/*.jpg` (~1.1 МБ); сырые `<img>` → `next/image`.
- [ ] Деплой: после финализации 3/4/5 — push в `main` → автодеплой Coolify (RobloxBank-Guide UUID `ebac6llpah5n2x58rb64yn8j`), затем проверить прод.

### Источники ассетов
Записи экрана владельца (Roblox Creator Hub, мобильный): путь до Questionnaire, ответы опроса, путь создания пасса, страница Sales. Нарезаны в `outputs` → положены в `public/guide/`.
