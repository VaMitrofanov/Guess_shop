...
### Деплой TWA (RF) — ТРЕБУЕТСЯ РУЧНОЙ ТРИГГЕР
RF (RobloxBankWeb) **не автодеплоится** (IP-блок GitHub). Коммит `1341275` запушен, но НЕ задеплоен — нужен Coolify-триггер с токеном владельца:
```bash
curl -X POST "http://89.110.94.117:8000/api/v1/deploy?uuid=z10ws7m1q45h281zwedmhei4&force=true" \
  -H "Authorization: Bearer $COOLIFY_TOKEN"
```
(У меня нет `COOLIFY_TOKEN`, поэтому деплой за владельцем. Coolify при ручном триггере сам подтянет код из GitHub и пересоберёт Next.js.)
