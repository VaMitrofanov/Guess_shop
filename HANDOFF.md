...
## Сессия 2026-07-02 (день-12) — Фича: перепривязка заказов к другому пользователю (TWA)

### Проблема

Покупатель активирует код с одного устройства/платформы (VK), забивает. Потом он сам или родственник пытается продолжить с другого аккаунта (TG). Заказ намертво привязан к первому юзеру — раньше решалось только руками через БД.

### Что сделано

Полная фича перепривязки заказов из TWA:

#### 1. API: поиск пользователей (`search-users`)

Новый action в `/api/twa/orders`. Автодетект ввода:
- `@username` → поиск по TG username
- Цифры → точное совпадение tgId/vkId
- Текст → ILIKE по username, name, robloxUsername

Лимит 10 результатов, сортировка по дате регистрации.

#### 2. API: перепривязка заказа (`rebind-order`)

Новый action в `/api/twa/orders`:
- Проверка статуса ∈ `[AWAITING_GAMEPASS, PENDING, IN_PROGRESS, ERROR]`
- Проверка что целевой юзер ≠ текущий
- **Транзакция:** `WbOrder.update(userId, platform)` + `WbCode.updateMany(userId)` — обе таблицы обновляются атомарно
- **Аудит:** в adminNote копится история: `[REBIND 2026-07-02] VK:anna → TG:@Max_Poverinov (родственник)`
- **Уведомление** новому юзеру через TG/VK API (twa-notify.ts):
  - Если AWAITING_GAMEPASS → сообщение + кнопка «📋 Инструкция»
  - Если PENDING/IN_PROGRESS → информирование без CTA (геймпасс уже отправлен)

#### 3. UI: кнопка «🔄 Перепривязать» в карточке заказа

Показывается на **всех** вкладках для заказов со статусом AWAITING_GAMEPASS, PENDING, IN_PROGRESS, ERROR. Расположена между блоком «Перевести/Редактировать» и ActionPanel.

#### 4. UI: RebindModal (двухшаговый)

**Шаг 1 — Поиск:** поле ввода с debounce (300ms), результаты — карточки юзеров с платформой (TG/VK), username и robloxUsername. Тап = выбор.

**Шаг 2 — Подтверждение:** карточка перехода `oldUser (VK) → newUser (TG)`, опциональная заметка, кнопка «Перепривязать».

### Файлы

| Файл | Что изменено |
|------|-------------|
| `src/app/api/twa/orders/route.ts` | +action `search-users`, +action `rebind-order` (транзакция WbOrder+WbCode, аудит, уведомление) |
| `src/app/twa/_components/screens/OrdersScreen.tsx` | +`RebindModal` (двухшаговый: поиск → подтверждение), +кнопка «🔄 Перепривязать» в OrderCard |
| `src/lib/twa-notify.ts` | +`notifyRebind()` — уведомление новому юзеру (TG inline-кнопка или VK текст) |

### Ожидает тестирования

- [ ] Перепривязка U1M0MWF: VK (anna.yuryevna91) → TG (@Max_Poverinov)
- [ ] Поиск по @username → результаты → выбор → подтверждение → перепривязан
- [ ] Уведомление новому юзеру (TG или VK)
- [ ] adminNote содержит аудит-запись `[REBIND ...]`
- [ ] WbCode.userId тоже обновлён
- [ ] Платформа заказа сменилась (VK→TG)
...
## Сессия 2026-07-02 (день-11) — Фикс: VK бот молчал на «💎 Ещё на ник» (и все прямые заказы)

### Инцидент

VK бот молчит при нажатии кнопки «💎 Ещё на viu1758» (и любой другой `start_direct`). Покупатель Алексей Асфатулин (viu1758, VK ID 256993747) не мог оформить прямой заказ.

### Root cause

`buildVkPackKb()` генерировала inline-клавиатуру с **12 кнопками** (10 пакетов + «✏️ Своё количество» + «❌ Отменить»). VK API ограничивает inline-клавиатуру **10 кнопками** максимум → `APIError: Code №911 - Keyboard format is invalid: keyboard contains too much buttons`. Ошибка всплывала в generic catch `message_new` — бот логировал, но пользователь не получал ответа.

Предыдущий фикс support-pause (коммит `e22097f`) был корректным и задеплоенным — но кнопки всё равно не работали из-за этого бага клавиатуры.

### Фикс

Новый массив `VK_PACKS = [100, 200, 300, 500, 800, 1000, 1500, 2000]` — 8 пакетов (убраны 400 и 1200, у них есть близкие соседи 300/500 и 1000/1500). «✏️» и «❌» объединены в одну строку. Пользователи по-прежнему могут ввести 400 или 1200 через «✏️ Своё количество».

При наличии `lastOrderAmount` (кнопка 🔄 reorder) — 7 пакетов + 1 reorder + ✏️ + ❌ = 10.
Без reorder — 8 пакетов + ✏️ + ❌ = 10.

Также добавлен catch-reply в `bot.ts` для `message_new`: при необработанной ошибке юзер получит «⚠️ Произошла ошибка» вместо молчания.

### Пользователь viu1758

- VK ID: 256993747, Roblox ник: viu1758, TG: нет
- Отправлено сообщение через VK API: «Бот на связи, жми кнопку» + inline «💎 Ещё на viu1758» / «В моё меню»

### Файлы

| Файл | Что изменено |
|------|-------------|
| `bots/vk/handlers.ts` | `buildVkPackKb()`: VK_PACKS (8 вместо 10), layout в ряды по 4, ✏️+❌ в одну строку |
| `bots/vk/bot.ts` | catch в `message_new` теперь отвечает «⚠️ Произошла ошибка» вместо молчания |

### Деплой

- [x] TypeScript — преexisting ошибки (vk-io types), мой код чист
- [x] Коммит `2354787` → `git push origin main`
- [x] VK бот — задеплоен, `Bot started ✅`, коммит `2354787`
- [x] Сообщение viu1758 отправлено (message_id: 5354)
- [ ] TG бот — не затронут
- [ ] Web — не затронут

### Coolify

Токен `29|v2di...` **просрочен** (`Unauthenticated`). Деплой VK бота выполнен через Coolify UI вручную. Нужно сгенерировать новый API-токен в Coolify Settings → API Tokens.

### Проверка flow прямых заказов (VK)

Полный flow прослежен по коду — после фикса клавиатуры все шаги работают:

1. `start_direct` → `handleStartDirect` → `buildVkPackKb` (теперь ≤10 кнопок) ✅
2. `direct_pack` → `handleDirectPackSelect` → показ цены/бонуса/подтверждения (4 кнопки) ✅
3. `direct_confirm` → `handleDirectConfirm` → шаг ника (2-3 кнопки) ✅
4. Ник → `handleVkDirectNickResolved` → поиск геймпассов → `direct_gp_pick` ✅
5. `direct_submit` → создание заказа ✅
6. Все промежуточные клавиатуры содержат ≤10 кнопок ✅
...
Не затронут. AVITO-заказы уже были доступны через `?status=AVITO`.
...
5 доработок экрана «Аккаунт» (`BossrobuxScreen.tsx`):

#### 1. Поиск — первая секция
Секция «Поиск и выкуп» перемещена на самый верх экрана (была третьей). Это главный рабочий инструмент — поиск геймпассов по нику/ID, проверка, выкуп.

#### 2. Баланс — чистые R$ в скобках
Баланс аккаунта теперь показывает чистые робуксы: `1 570 R$ (1 099 чистых)`.
Формула: `Math.floor(balance * 0.7)` — то, что реально можно выплатить продавцам.

#### 3. Cookie — внутрь секции Аккаунт
Отдельная секция «Cookie» убрана. Вместо неё — кнопка «🔑 Cookie» в секции Roblox-аккаунт. По нажатию раскрывается textarea + save. Экономит место на экране.

#### 4. Авито через поиск (интеграция)
Отдельная секция «+ Авито заказ» убрана. Теперь в результатах поиска рядом с кнопкой «🛒» (выкупить) есть кнопка «Авито» (оранжевая). Один клик — создаёт заказ Авито с данными найденного геймпасса (gamepassUrl, sellerName, amount = floor(price * 0.7)).

#### 5. Бэкфилл purchaserUsername
SQL-бэкфилл: 191 COMPLETED заказов без `purchaserUsername` → проставлен `bendi7038` (единственный куки-аккаунт). Теперь все заказы в «Истории покупок» группируются по `bendi7038`, а не в группе «Ручные».

### Новый порядок секций Аккаунта

```
Аккаунт (BossrobuxScreen)
├── 🔍 Поиск и выкуп          ← ПЕРВЫЙ
│   ├── SegmentControl (по нику / по ID)
│   ├── Input + 🔍
│   └── Результаты: GamepassCard
│       ├── 🛒 (выкупить)
│       └── Авито (создать заказ)
│
├── 👤 Roblox-аккаунт
│   ├── Имя, ID, Баланс (чистые в скобках)
│   ├── 🔄 Обновить баланс
│   └── 🔑 Cookie (toggle → textarea)
│
├── 📋 К выкупу
│   └── BuyoutSection
│
└── 📊 История покупок
    └── TransactionHistory (группировка по bendi7038)
```
...
| Файл | Что изменено |
|------|-------------|
| `src/app/twa/_components/screens/BossrobuxScreen.tsx` | Перестановка секций, баланс с чистыми, cookie inline, Авито в поиск, удаление CreateAvitoSection |

### БД

- Бэкфилл: `UPDATE "WbOrder" SET "purchaserUsername" = 'bendi7038' WHERE status = 'COMPLETED' AND "purchaserUsername" IS NULL` — 191 строка

### Деплой

- [x] TypeScript чисто (`npx tsc --noEmit`)
- [x] Бэкфилл purchaserUsername — выполнен (191 строка)
- [ ] Коммит + push
- [ ] Web (RF) — автодеплой
- [ ] TG/VK боты — не затронуты
- [ ] **Визуальная проверка в TWA** — ожидает
...
```
Аккаунт (BossrobuxScreen)
├── Roblox-аккаунт (cookie, баланс)
├── Cookie (textarea)
├── Поиск и выкуп (если cookie)
├── К выкупу (если cookie)
└── 📋 История покупок ← НОВОЕ (всегда видна)
```
...
- `gamepassName String?` в WbOrder → название ГП вместо ID
- Thumbnail (Roblox CDN, lazy-load)
- Фильтры по периоду / нику продавца
- Суммарная статистика (за неделю/месяц)
...
## Сессия 2026-07-01 (день-2) — Размерный редизайн TWA + фикс сортировки «К выкупу» (коммит `6452a14`)

### Что сделано

Два фикса для TWA под целевые устройства (iPhone 14/15 Pro Max + MacBook 16"):

#### 1. Размерный редизайн — все шрифты увеличены

Мелкий шрифт на крупных экранах выглядел как ошибка. Все элементы TWA пересмотрены:

| Элемент | Было | Стало |
|---------|------|-------|
| Ник в карточке | 16px | **17px** |
| Сумма R$ | 20px | **22px** |
| Таймер ⏱ | 15px | **16px** |
| R$ / разделители | 13px | **14px** |
| Data rows (ник, ссылка, код) | 15px | **16px** |
| Кнопки действий | 14px, pad 12px | **15px, pad 14px** |
| Табы-фильтры | 14px, pad 7×14 | **15px, pad 9×16** |
| CopyBtn | 12.5px | **14px** |
| Заметка textarea | 14px | **15px** |
| MoveToModal | 12–13px | **14–15px** |
| Поиск input | 14.5px | **16px** |
| MiniDashboard лейбл/сумма | 10.5/18px | **12/20px** |
| AccountScreen InfoRow | 15px | **16px** |
| SegmentControl | 13px | **15px** |
| GamepassCard | 14/12/13px | **16/14/15px** |
| BuyoutSection суммы | 18px | **22px** |
| BuyoutSection ник | 15px | **17px** |
| Cookie textarea | 13px | **15px** |
| BottomNav labels | 10px | **11px** |

Все touch-таргеты (кнопки, табы, чипы) увеличены в padding. Минимальный контентный шрифт — 14px, бейджи/декоративные — 12px.

#### 2. Фикс сортировки «К выкупу»

**Проблема:** вкладка BUYOUT сортировала по `createdAt`, а не по моменту попадания заказа в раздел.

**Решение:**
- **Сортировка:** BUYOUT и DIRECT табы теперь сортируются по `[pendingAt ASC, createdAt ASC]` — заказы идут в порядке попадания в раздел, самые старые первыми
- **Таймер:** единый `createdAt` во ВСЕХ вкладках (как в «Все»), убрано ветвление `pendingAt ?? createdAt` для BUYOUT/DIRECT

### Файлы

| Файл | Что изменено |
|------|-------------|
| `src/app/api/twa/orders/route.ts` | `sortForTab()` → `orderByForTab()`: BUYOUT/DIRECT сортируются по `[pendingAt, createdAt]` |
| `src/app/twa/_components/screens/OrdersScreen.tsx` | Размерный редизайн всех шрифтов/padding + таймер = `createdAt` |
| `src/app/twa/_components/screens/BossrobuxScreen.tsx` | Размерный редизайн + таймер = `createdAt` |
| `src/app/twa/_components/BottomNav.tsx` | Метки 10→11px, бейджи 9→10px, padding увеличен |

### Деплой

- [x] TypeScript чисто
- [x] Коммит `6452a14` → `git push origin main`
- [ ] **Force-deploy Web (RF) — вручную из Coolify UI**
- [ ] TG/VK боты — не затронуты

---

## Сессия 2026-07-01 — Полный реворк вкладок Orders TWA + UI + «К выкупу» в Аккаунте

### Целевые устройства для вёрстки TWA

TWA используется с **iPhone 14 Pro Max**, **iPhone 15 Pro Max (×2)** и иногда **MacBook Pro 16"** в полноэкранном режиме.
Шрифты, паддинги и touch-таргеты нужно верстать под эти крупные экраны — мелкий шрифт здесь выглядит как ошибка.

### Что сделано

Полная переработка системы вкладок в OrdersScreen TWA. Вместо 9 статусных табов — 8 логических разделов. + UI-рефреш карточек + секция «К выкупу» в экране Аккаунт.

### Коммиты (все на main, запушены)

| Коммит | Описание |
|--------|----------|
| `6169014` | Полный реворк вкладок: 7 табов, виртуальные фильтры, избранное, ERROR-автостатус, компактные карточки |
| `81c3921` | +вкладка «Готово» (DONE), фикс грязных робуксов (amount / 0.7, было * 0.7) |
| `f67c374` | Fix краш при поиске — `counts` может быть `null`, добавлен `?.` |
| `575a044` | UI крупнее (шрифты, паддинги), чистые R$ в скобках на «К выкупу», секция «К выкупу» в Аккаунте |
| `ccafe42` | Прямые заказы первыми в «К выкупу» (Аккаунт), бейдж «Прямой» |

### Новая структура вкладок (8 табов)

| Таб | Фильтр | Сортировка | Панель кнопок | Цена |
|-----|--------|------------|---------------|------|
| **Все** | все заказы | новые сверху | нет | чистые |
| **К выкупу** | PENDING + IN_PROGRESS, non-direct, non-favorite | старые сверху | Выкупить / Ошибка / Выкуплено / ✕ | грязные (чистые) |
| **Прямой** | direct orders, active statuses, non-favorite | старые сверху | Выкупить / Ошибка / Выкуплено / ✕ | грязные |
| **Новые** | AWAITING_GAMEPASS, < 40ч, non-favorite | новые сверху | нет | чистые |
| **Ошибка** | ERROR status, non-favorite | старые сверху | Выкупить / Выкуплено / ✕ | грязные |
| **Ждут ссылку** | AWAITING_GAMEPASS, ≥ 40ч, non-favorite | старые сверху | нет, ручной перевод | чистые |
| **Готово** | COMPLETED | новые сверху | нет | чистые |
| **Избранное** | isFavorite = true | новые сверху | ручной перевод | чистые |

### Робуксы — важно!

- `amount` в БД = **ЧИСТЫЕ** робуксы (что получает продавец)
- **Грязные** = `Math.ceil(amount / 0.7)` — номинал геймпасса (Roblox забирает 30%)
- Грязные показываются в: К выкупу, Прямой, Ошибка
- Чистые показываются во всех остальных
- На вкладке «К выкупу» дополнительно чистые в скобках: `1 200 R$ (840)`

### UI-рефреш карточек

Увеличены шрифты и отступы для комфортной работы на крупных iPhone:
- Ник: 14 → **16px**
- Сумма: 17 → **20px**
- Data rows: 14 → **15px**
- Кнопки: 13 → **14px**, padding 10→12, borderRadius 10→12
- Табы: 13 → **14px**, padding и бейджи увеличены
- Карточки: padding 12/14 → **14/16**, gap между строками увеличен

### Секция «К выкупу» в экране Аккаунт (BossrobuxScreen)

Новая третья секция после «Поиск и выкуп». Загружает параллельно DIRECT + BUYOUT заказы.
- **Прямые заказы первыми** (приоритет), с бейджем «Прямой»
- Компактные карточки: ник, таймер, грязные/чистые R$, WB-код, кнопка «Выкупить»
- Покупка прямо из экрана Аккаунт без переключения на Заказы
...
4. Всё применено через `prisma db push`
...
**GET — виртуальные фильтры (8 бакетов):**
...
Counts — 1 SQL с FILTER (WHERE ...) для всех 8 бакетов. Сортировка: BUYOUT/DIRECT/ERROR/AWAITING_LINK = ASC, остальные = DESC.

**POST — новые actions:** `toggle-favorite`, `set-error`, `move-to` (+ заметка). `purchase` → auto-ERROR при ошибке.

### Бот-код изменения

- `bots/tg/handlers.ts` — `pb:` handler: auto-ERROR при ошибке; принимает ERROR
- `bots/tg/handlers.ts` — `admin_ok:` handler: принимает ERROR
...
| `src/app/api/twa/orders/route.ts` | 8 виртуальных фильтров, автосортировка, новые actions, purchase → auto-ERROR |
| `src/app/twa/_components/screens/OrdersScreen.tsx` | 8 табов, UI-рефреш (крупные шрифты), чистые в скобках, ActionPanel, MoveToModal |
| `src/app/twa/_components/screens/BossrobuxScreen.tsx` | +секция «К выкупу» (DIRECT первыми + BUYOUT), кнопка покупки |
| `bots/tg/handlers.ts` | pb: → auto-ERROR, admin_ok: принимает ERROR |
| `bots/tg/admin/hub-orders.ts` | active/batch views включают ERROR |
...
- [x] TypeScript чисто
- [x] Prisma db push применено
- [x] Коммиты запушены на main
- [ ] Force-deploy Web (RF) — **Coolify токен просрочен**, нужен ручной редеплой из UI
- [ ] TG бот — не требует редеплоя (бот-код не менялся в этой сессии)
- [ ] VK бот — не затронут
...
Формула расчёта цены для кастомных сумм (не из фиксированного перечня паков). Заменяет старый плоский fallback `amount × 0.7`.

**Формула:** `курс × количество` (+60 ₽ наценка только для 100–499)

| Диапазон R$ | Курс (₽/R$) | Наценка |
|---|---|---|
| 100–499 | 1.0 | +60 ₽ |
| 500–999 | 0.9 | — |
| 1000–1499 | 0.8 | — |
...
Формула применяется ВСЕГДА — и для кастомных сумм, и для паков. Таблица `DIRECT_PRICES` больше не переопределяет цену, только задаёт список кнопок.

**Изменившиеся цены паков:** 400 R$ (440→460₽), 500 R$ (460→450₽), 1200 R$ (890→960₽). Остальные без изменений.

**Ответ покупателю:**
- < 1000 R$: только стоимость в рублях
- ≥ 1000 R$: строка «📊 Курс: X ₽/R$» + стоимость

**Лимиты:** CUSTOM_MIN=100, CUSTOM_MAX=100 000 (было 200–10 000).

### Файлы

| Файл | Что изменено |
|---|---|
| `bots/shared/admin.ts` | +`customRate()`, обновлён `directPrice()` (тиерная формула вместо `*0.7`), `CUSTOM_MIN=100`, +`CUSTOM_MAX=100_000` |
| `bots/tg/handlers.ts` | Импорт `customRate/CUSTOM_MAX/DIRECT_PRICES`, лимиты валидации, промпт `dp:custom`, строка курса в `handleDirectPackChosen` |
| `bots/vk/handlers.ts` | Зеркало TG: импорт, лимиты, промпт, строка курса в `handleDirectPackSelect` |

### Примеры расчёта

| Сумма | Диапазон | Курс | Формула | Цена | Ответ |
|---|---|---|---|---|---|
| 132 R$ | 100–499 | 1.0 | 1.0×132+60 | 192 ₽ | только цена |
| 413 R$ | 100–499 | 1.0 | 1.0×413+60 | 473 ₽ | только цена |
| 623 R$ | 500–999 | 0.9 | 0.9×623 | 561 ₽ | только цена |
...
Первое сообщение нового пользователя (без кода, не returning) теперь содержит подписку на канал/сообщество с мотивацией: «бот точнее находит геймпассы по нику + персональная инструкция — заказ оформляется прямо в ней».

**TG:** inline-кнопка «🔔 Подписаться» + «💎 Купить напрямую» (заменяет reply-клавиатуру `menuReplyKb`).
**VK:** кнопки «🔔 Подписаться» + «✅ Я вступил» + «💎 Купить напрямую».

### 2. WB-код и номинал в гейте подписки (TG + VK)

Когда пользователь активирует код, но не подписан — сообщение подписки теперь содержит заголовок:
```
📦 Заказ XXXXXXX · 500 R$ — создан
```
Админ видит код прямо в чате — можно сразу искать в TWA.

### 3. Мотивация подписки унифицирована

Все точки гейта (TG: registerStart, handleWbCodeTextEntry, idle; VK: sendVkSubPrompt) используют одну формулировку:
- «бот точнее находит геймпассы по нику» + «персональная инструкция — заказ оформляется прямо в ней»

### 4. Отмена прямого заказа покупателем (TG + VK)

Покупатель может сам отменить прямой заказ в статусе AWAITING_PAYMENT (реквизиты ещё не отправлены):

**TG:** кнопка «❌ Отменить заказ» (`ucd:${orderId}`) в `buildStatusMessage`.
**VK:** кнопка «❌ Отменить заказ» (`user_cancel_direct` payload) в статус-экране.

При отмене:
- Статус → REJECTED, причина «Отменён покупателем»
- Бонус R$ возвращается на баланс пользователя
...
## Сессия 2026-06-29 (день-2) — Гейт подписки: «бот выдаст инструкцию» (✅ ЗАДЕПЛОЕНО, коммит `a1bb18a`)

### Контекст

Гейт подписки на канал (TG) / сообщество (VK) позиционировал подписку как «бонусы и акции» — покупатели не понимали, зачем подписываться. Новый тон: подписка = обязательный шаг, после которого **бот выдаст персональную инструкцию** и можно оформить заказ по нику в Roblox.

### Что изменено

**TG бот (`bots/tg/handlers.ts`) — 3 гейта:**

1. **`registerStart` (строка ~527)** — активация кода по deep link:
   - Было: «Подпишись на наш канал — там статус заказов, бонусы и акции»
   - Стало: «Чтобы продолжить, подпишись на наш канал 👇 После подписки бот выдаст тебе **персональную инструкцию** и ты сможешь оформить заказ по нику в Roblox.»

2. **`handleWbCodeTextEntry` (строка ~2316)** — активация кода текстом:
   - Зеркально registerStart

3. **Idle-гейт (строка ~1271)** — для unsubscribed юзеров без активного кода:
   - Было: «У наших клиентов есть закрытый канал — уведомления, бонусы и акции. Загляни»
   - Стало: «Подпишись на наш канал, чтобы продолжить 👇 После подписки бот выдаст тебе инструкцию и ты сможешь оформить заказ.»
   - Кнопка: «⭐ Стать участником» → «⭐ Подписаться» (унифицировано)

**VK бот (`bots/vk/handlers.ts`) — 2 точки:**

1. **`sendVkSubPrompt` (строка ~460)** — общий промпт для всех VK-гейтов (`handleRefActivation`, `handleStartDirect`, `handleIdleMessage`):
   - Было: «Ты в одном шаге! У наших клиентов есть закрытое сообщество — анонсы акций, розыгрыши и бонусы»
   - Стало: «Чтобы продолжить, подпишись на наше сообщество 👇 После подписки бот выдаст тебе персональную инструкцию и ты сможешь оформить заказ по нику в Roblox.»

2. **`check_sub` success без AWAITING_LINK (строка ~604):**
   - Было: «Спасибо за подписку! Теперь ты можешь активировать свой код»
   - Стало: «Подписка подтверждена! Теперь отправь свой код — бот выдаст инструкцию.»

**Не тронуто:**
- Soft-промпты после оформления заказа (non-blocking «кстати, подпишись на акции») — они не блокируют
- `chat_member` handler (TG, пост-подписочное сообщение) — его текст уже был правильный
- `check_sub` с AWAITING_LINK (VK) — его текст уже правильный

### Принцип

Подписка = обязательный гейт на входе, но мотивация не «бонусы», а **«бот выдаст инструкцию»**. Покупатели должны понимать: подписался → бот сам напишет → всё дальше через бота.

### ✅ Деплой (прод-проверка 2026-06-29)

- [x] Коммит `a1bb18a` → `git push origin main`
- [x] VK bot (`gmtpfqosgoz23vjyxyczuic9`) — image tag `a1bb18a`, Up 6 min, `Bot started ✅ (group 237309399)`
- [x] TG bot (`lyz78enntugna9em1biopinr`, SG) — image tag `a1bb18a`, `Bot started ✅`, `Bot profile applied ✅`, все 3 cron started ✅
- [x] Ошибок в логах нет
