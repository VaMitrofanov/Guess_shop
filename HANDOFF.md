...
- **Мёртвые text-interceptors** в `bots/tg/admin/index.ts` (`bot.hears` старых reply-кнопок + pending-input хендлеры) — Phase C предполагал их удаление «после стабилизации Phase B», но **Phase B не завершён** (B1 Stats, B5 cleanup ⏳). Хабы ещё достижимы через inline-callback'и → удалять рано, риск убрать живой доступ. Делать вместе с финалом Phase B.
