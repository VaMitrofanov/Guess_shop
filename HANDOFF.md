...
## Сессия 2026-06-23 (вечер) — Меню покупателя (мини-профиль) + чистка велкома после ввода кода (✅ ЗАДЕПЛОЕНО, `60ecc21`)

> Первая порция «большого промта с фиксами» после ручного прогона `TEST300`. Все правки — `bots/tg/handlers.ts` + `bots/shared/admin.ts`. tsc по `bots/tsconfig.json` — 0 новых ошибок в этих файлах. TG_bot передеплоен (Coolify deploy `bskin7yqx3erxkju1l156q9c`, status finished, commit `60ecc21`, container running).
...
- **VK не трогал** — велком-чистка и меню сделаны только в TG (пользователь явно просил «мини-профиль в тг»). Зеркалить в VK — по запросу.
