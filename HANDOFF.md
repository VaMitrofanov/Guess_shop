...
### Осталось / сознательно отложено
- **Phase C долг** (`updateMainMenu` no-op в `bots/tg/admin/menu.ts:57` + call-sites `handlers.ts:2131`, `hub-orders.ts:523`) — чистый tech-debt в коде ботов, **не трогал**: невидимо пользователю + риск при деплое ботов. Удалять при следующей правке ботов.
