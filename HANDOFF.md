...
> Коммит: `3287acc`. Деплой: Web (ручной + авто по push), Guide вручную, TG+VK боты передеплоены. Все 4 контейнера на `3287acc`.
...
Показывается в сообщении «Твой геймпасс принят» (TG+VK) — между «просто жди сообщение» и «Код ВБ».

### Деплой
- Коммит `3287acc` → `git push`. Web: ручной force-deploy (`wbjzi6xsie93grklv0nt76on`). Guide вручную (`cbl12d6owst2ksx3dujjpurh`). TG (`aeo5v68bp4nvsdlcqu7s2irh`) + VK (`qi2z43xm3ns0fqvd71x6pn1d`) передеплоены.
...
Когда Roblox стабилизируется — удалить или закомментировать `ROBLOX_DELAY_BANNER` и его вставку в acceptance-сообщении:
- `bots/tg/handlers.ts` ~строка 765: const + строка ~1984: `+ ROBLOX_DELAY_BANNER +`
- `bots/vk/handlers.ts` ~строка 2019: const + строка ~1362: `+ ROBLOX_DELAY_BANNER +`
