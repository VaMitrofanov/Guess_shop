...
**Проблема:** страница заказов (`/twa`, вкладка «Заказы») не загружалась или грузилась 4+ секунд.

**Корневая причина:** API `/api/twa/orders` выполнял ~11 последовательных SQL-запросов через пул с `max: 1` соединением → все запросы стояли в очереди друг за другом.

**Что изменилось:**

1. **`src/lib/prisma.ts` — пул соединений: 1 → 3**
   - Было: `max: 1`, `idleTimeoutMillis: 10000`, `connectionTimeoutMillis: 10000`
   - Стало: `max: 3`, `idleTimeoutMillis: 30_000`, `connectionTimeoutMillis: 15_000`
   - Теперь `Promise.all` реально параллелит запросы, а не ставит в очередь.

2. **`src/app/api/twa/orders/route.ts` — оптимизация запросов**
   - **Счётчики для чипсов:** 7 отдельных `COUNT` запросов → 1 raw SQL с `COUNT(*) FILTER` (без поиска) или 1 Prisma `groupBy` (с поиском). Раньше была ссылка на несуществующую функцию `buildSearchSQL` — сломанный код.
   - **Cluster numbering + review status:** 3 запроса шли последовательно (2 фазы). Теперь все 3 в одном `Promise.all`.
   - **skipCounts:** при пагинации (load more) счётчики чипсов не перезапрашиваются — фронтенд сохраняет предыдущие.
   - **Баг `total` → `finalTotal`:** ответ API возвращал `total` из счётчиков вместо `finalTotal`, который учитывает `skipCounts`. Пагинация работала неправильно.
   - **Итого:** ~11 последовательных запросов → 4-5 запросов в 2 параллельных батча.

3. **`src/app/twa/_components/screens/OrdersScreen.tsx` — кэш в sessionStorage**
   - При первом открытии вкладки данные сохраняются в sessionStorage (TTL 60 сек).
   - При повторном открытии: кэшированные данные показываются мгновенно, свежие догружаются в фоне.
   - Нет спиннера при переключении табов, если кэш свежий.

**Результат:** время загрузки страницы заказов: ~4 сек → <1 сек (при тёплом Neon). Повторные открытия — мгновенно (кэш).
