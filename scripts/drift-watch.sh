...
#   * повторно лечить не раньше COOLDOWN — если выкатка не помогла, проблема не
#     в том, что её мало запускали, и человека надо звать, а не крутить сборку
#     каждые 15 минут.
...
heal_guide() {
  local busy uuid deployment last age
  ...
  if [[ -f "$HEAL_STATE" ]]; then
    last="$(cat "$HEAL_STATE" 2>/dev/null || echo 0)"
    age=$(( $(date +%s) - ${last:-0} ))
    if (( age < HEAL_COOLDOWN_SEC )); then
      echo "$STAMP heal: прошлая выкатка $age с назад не помогла — нужен человек"
      return 3
    fi
  fi
...
Автовыкатка Guide уже была и не помогла. Нужен человек:
