...
async function main() {
  ...
  const bonusMsg =
    ...
    `Это бонус к следующей прямой покупке в боте или на robloxbank.ru.\n` +
    `Отдельно он не выдаётся и к покупке на WB не применяется; к номиналу прямого заказа добавится автоматически.`;
