...
- деплой ставится **только при пустой очереди Coolify** (`application_deployment_queues`);
  ...
  тика, без алерта;
- повторно лечить не раньше `DRIFT_HEAL_COOLDOWN_SEC` (по умолчанию час). Если выкатка не
  помогла, проблема не в том, что её мало запускали, — приходит алерт «нужен человек».
