...
| `WB_DBS_AUTO_RECEIVE` | TG | Не задан ⇒ доставку на WB закрывает оператор вручную (окно ~1 час!), и до её закрытия покупателю не уходит гейт. Даже включённым он действует только на заказы моложе четырёх часов — см. ниже |
| `WB_DBS_AUTO_GATE` | TG | Не задан ⇒ ручной режим: код ловится сам, гейт выпускает оператор |

С 21.08.2026 автозакрытие ограничено возрастом заказа: код, пришедший позже **четырёх
часов** с момента оформления, доставку не закрывает — в админку уходит сообщение с самим
кодом, решение принимает человек (`WB_AUTO_RECEIVE_ORDER_AGE_MS`, разбор — в
[wb-dbs-delivery-plan.md](wb-dbs-delivery-plan.md)). Правило живёт в коде и флагом не
регулируется; выключенный `WB_DBS_AUTO_RECEIVE` просто распространяет это поведение на
все заказы.

С 21.08.2026 гейт уходит покупателю **только за закрытой доставкой** — и автоматом, и
кнопкой в консоли. Практическое следствие для `WB_DBS_AUTO_RECEIVE=false`: покупатель
получит свой код не в момент присылки кода доставки, а после того, как оператор закроет
заказ (своей кнопкой «Завершить» или в кабинете WB — тогда выдачу подхватит проход
`auto-gate` на следующем цикле после опроса статусов). Подробности —
...
Web и Guide, он ставит деплой Guide через artisan. Крон на хосте стоит на `*/15`, поэтому
Guide догоняет Web в течение 15 минут после выкатки — сам.

Практическое следствие, проверенное 06.09: запускать `deploy-web-and-guide.sh --guide-only`
сразу после push **больше не нужно и вредно** — drift-watch уже поставил свою сборку, а
скрипт ставит вторую того же коммита (деплои 1446 + 1447, второй отменён руками). Порядок
теперь такой: push → дождаться webhook-сборок Web/ботов → **дождаться тика drift-watch** →
сверить отпечатки. Ручной `--guide-only` остаётся только для случая, когда тик прошёл, а
Guide не обновился.

Токен Coolify перевыпущен 06.09 (`31|…`, старый `30|…` отдавал 401 с начала сентября) —
`deploy-web-and-guide.sh` снова работает, но по причине выше нужен редко.

> ⚠️ **Алерты drift-watch не доходят вообще.** С 27.07 в логе **3046** строк
> `alert to <id> failed`: `curl: (28) Failed to connect to api.telegram.org` — RF-хост
> Telegram не видит (ровно то, из-за чего боты живут за SG-мостом), а скрипт ходит туда
> напрямую. Лечащая часть монитора работает, **сигнальная мертва шесть недель**: если
> самолечение не справится, об этом никто не узнает. Чинится отправкой через `/tg-proxy`
> на мосту. Не сделано.
...
### 🔴 Причина «молчащего webhook» найдена: watch-paths (07.09.2026)

Записи выше («автодеплой не сработал», `--force-web` как штатный путь) описывали
симптом. Причина — не доставка webhook, а **фильтр путей у самого приложения**.

Проверено через `GET /api/v1/applications/{uuid}` → поле `watch_paths`:

| Приложение | Что стоит | Что это значит |
|---|---|---|
| `RobloxBankWeb` | `src/**`, затем **`!src/app/guide/**`** и `!src/app/api/wb-**`, плюс `public/**`, `prisma/**`, конфиги | коммит, который трогает ТОЛЬКО инструкцию, Web не будит |
| `RobloxBank-Guide` | `src/app/guide/, src/app/api/wb-` + `next.config.guide.ts, Dockerfile.guide` — **через запятую в одну строку** | Coolify ждёт по глобу на строку, поэтому Guide не будит ничего |

Вместе: **guide-правка не поднимала ни Web, ни Guide.** Отсюда «webhook молчит»
после `dafe969`, `ad0bde7` и вечный `--force-web` после guide-коммитов.
Маскировало это то, что в коммите обычно оказывался ещё какой-нибудь файл под
`src/**` — тогда Web собирался «сам».

**Кто что отдаёт — перемерено 07.09 по `x-robloxbank-guide-release`** (отпечаток
Web и Guide различается, это надёжнее догадок о правиле Traefik):

| Адрес | Контейнер |
|---|---|
| `/` | Web |
| `/guide?source=site&amount=…` (покупка на сайте) | **Web** |
| `/guide?source=wb` | Guide |
| `/guide?source=wb&skip=1&code=…` (личная ссылка WB и ботов) | **Guide** |
| `/guide?source=wb&preview=1&…` | Guide |

То есть `Query(source, wb)` совпадает по НАЛИЧИЮ параметра, а не по точному
запросу: все ссылки с `source=wb` уходят в Guide. Прежняя запись в заметках
(«ссылки с `code`/`skip` уходят в Web») **неверна** — проверять надо отпечатком.

**Почему исключение у Web всё равно вредно.** Гейт с `source=wb` Web не отдаёт,
зато отдаёт `/guide?source=site` — экран проверки аккаунта в покупке на сайте, и
это тот же код под `src/app/guide/**`. С исключением правка инструкции не
поднимала Web, и покупка на сайте продолжала работать на старой сборке, пока в
коммит случайно не попадал другой файл. **07.09 исключение `!src/app/guide/**`
снято.** `!src/app/api/wb-**` оставлено: эти роуты обслуживает Guide.

**Guide намеренно остаётся без автодеплоя** — не «сломан», а так задумано: две
параллельные сборки на RF-хосте (3.9 ГБ) валят друг друга на `npm ci`. Его
поднимает `scripts/deploy-web-and-guide.sh --guide-only` после Web и лечит
`drift-watch`. Строку `watch_paths` у Guide не трогали: сейчас она не работает
по синтаксису, и это совпадает с намерением, но полагаться на опечатку нельзя —
начни Coolify её разбирать, Guide пойдёт собираться параллельно с Web.

### Боты показывали `running:unknown` — у них не было пробы живости (07.09.2026)

Coolify держал у `TG_bot` и `VK_bot` `health_check_enabled: false`, поэтому статус
обоих был `running:unknown`: панель не знала о них ничего, кроме того, что
контейнер запущен. Проверять было и нечем — все роуты моста закрыты
`VALIDATOR_KEY` и на GET без него отвечают 401/404, а для healthcheck это такой же
«нездоров», как мёртвый процесс.

Оба бота поднимают общий мост (`startBridgeServer`, порт 3000, `VALIDATOR_KEY`
задан у обоих), поэтому проба добавлена туда одна на двоих: `GET /healthz` → 200,
**до** проверки ключа. Она доказывает ровно одно — процесс жив и цикл событий
отвечает; что бот действительно разгребает очередь Telegram или VK, она не
доказывает и не притворяется.
...
### Guide чинится сам (03.09.2026)

Coolify по push в main поднимает Web и обоих ботов, а Guide — **нет**: у него намеренно нет
webhook, потому что две параллельные сборки на RF-хосте валят друг друга (резолв
`docker.io/docker/dockerfile:1.7`, инцидент 1396). Значит после каждой выкатки Web точка входа
с Wildberries остаётся на старом коде, пока человек не запустит `--guide-only`. За месяц это
дало **765 одинаковых алертов** в логе `drift-watch`: монитор превратился в шум, а гейт
неделями обслуживался чужим кодом.

`scripts/drift-watch.sh` теперь не только жалуется, но и лечит — с двумя ограничителями,
ради которых Guide и держали на ручном управлении:

- деплой ставится **только при пустой очереди Coolify** (`application_deployment_queues`);
  занятая очередь — это обычно как раз выкатка Web, и тогда монитор молча ждёт следующего
  тика, без алерта;
- повторно лечить не раньше `DRIFT_HEAL_COOLDOWN_SEC` (по умолчанию час) — **но только под
  тот же релиз Web**. Первый же боевой прогон (03.09, 11:15) показал, зачем эта оговорка: в
  10:15 монитор вылечил Guide сам, а через час приехал новый коммит — и пауза отказалась его
  лечить словами «прошлая выкатка не помогла», хотя та как раз помогла. Признак «не
  помогла» — это ТОТ ЖЕ отпечаток `X-RobloxBank-Guide-Release`, под который уже выкатывались;
  в файле состояния лежит `<время> <отпечаток>`.

**Алерты с RF-хоста доходят не всегда:** в логе регулярно `Failed to connect to
api.telegram.org` — лечение от этого не зависит (отправка best effort), но полагаться на
«мне не писали, значит всё хорошо» нельзя. Правда — в `/var/log/drift-watch.log`.

Токен Coolify для этого не нужен (он и просрочен): деплой ставится через `php artisan tinker`
внутри контейнера панели — тот же путь, которым деплоит её кнопка. UUID приложения ищется по
имени в `coolify-db`, поэтому в репозитории нет ни токенов, ни UUID.

Установка на хосте (крон переведён на 5 минут — раньше частота ничего не решала, потому что
проверка только жаловалась, а теперь она задаёт, сколько гейт живёт на чужом коде):

```
*/5 * * * * /root/drift-watch.sh >> /var/log/drift-watch.log 2>&1
```

Выключается одним env: `DRIFT_HEAL=0` — тогда скрипт ведёт себя как раньше.
...
**`VALIDATOR_SOURCE_URL` обязателен и для Web.** С 28.08.2026 через мост идёт весь
Roblox-трафик, а не только проверка одного пасса: без него поиск по нику на сайте
выбирает бюджет ретраев впустую и отвечает «пользователь не найден». Порядок выкатки при
изменении контракта моста — **сначала SG (мост), потом Web и боты**: новые эндпоинты должны
существовать раньше, чем в них начнут ходить.

**Сборки на RF разносить по одной.** 28.08.2026 push поднял три сборки сразу, и обе на RF
упали с `failed to resolve source metadata for docker.io/docker/dockerfile:1.7: lease does
not exist` — параллельные сборки затирают друг другу lease в кэше BuildKit (Docker Hub при
этом доступен, память ни при чём). Лечение на хосте: `docker builder prune -af`, затем
`docker pull docker/dockerfile:1.7`. Учти побочный эффект: prune сносит и cache mount
`/root/.npm`, поэтому первая сборка после него может молча оборваться на `npm ci` — повтор
на прогретом кэше проходит. Прежнее правило «Guide не собирать рядом с Web» расширяется на
все четыре сервиса.
...
### Guide: две ловушки, обе проверены 06.09.2026

**1. Параллельная сборка вешает Guide.** На прод-хосте 3.9 ГБ памяти. Если запустить
сборку Guide, пока собирается Web, сборка Guide встаёт на `npm ci` и висит молча:
статус остаётся `in_progress`, а логи не двигаются (признак — `updated_at` не меняется
дольше пары минут). Лечится так: `POST /api/v1/deployments/{uuid}/cancel` в Coolify,
дождаться пустой очереди и запустить Guide одному. Порядок «Web → дождаться → Guide»
не рекомендация, а условие.

**2. `Query(source, wb)` совпадает только с ТОЧНЫМ запросом.** Правило роутера
`guide-app` — `Host(robloxbank.ru) && Path(/guide) && Query(\`source\`, \`wb\`)`
(priority 100, у Web `main-web` priority 1). Проверено на живом хосте: `/guide?source=wb`
уходит в контейнер Guide, а `/guide?source=wb&skip=1&code=…` — **в Web**. То есть
реальные ссылки из WB-карточки и из бота обслуживает Web, а Guide держит только голый
`/guide?source=wb`, `/_next-guide/*` и API коридора (`/api/wb-code`, `/api/wb-link`).

Практический вывод: отставший Guide ломает не саму инструкцию, а **API коридора** —
и это по-прежнему повод держать Web и Guide на одной сборке. Проверять отставание надо
по образу контейнера, а не по содержимому страницы с параметрами:
`docker ps --format '{{.Names}}\t{{.Image}}' | grep ebac6ll`.
