...
- На каждой карточке результата поиска — кнопка **📎** → модалка `AttachOrderModal`:
  список заказов в статусах `AWAITING_GAMEPASS` / `REJECTED` / `ERROR` (30 последних,
  `action: "attachable-orders"`), фильтр по коду ВБ/нику/имени, у каждого заказа — сверка
  требуемой цены `ceil(amount / 0.7)` с ценой геймпасса (✓ зелёная / ⚠️ оранжевая + плашка
...
- Клиент получает то же уведомление, что и от бота: «🎉 Отлично, геймпасс принят!» + номер
  заявки (`notifyGamepassAttached`, TG или VK).
