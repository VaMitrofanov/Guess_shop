...
4. **Возврат фокуса во вкладку больше не перечитывает сессию** (`refetchOnWindowFocus`).
   На витрине это была чистая трата: `jwt`-callback обновляет в токене только `role` и
   `invalidated`, а `Navbar` из всей сессии читает ровно их («вошёл» и «админ») — то есть
   запрос ходил в базу, чтобы нарисовать те же две ссылки. Вход и выход в соседней
   вкладке этим не ломаются: их разносит `BroadcastChannel`, отдельным слушателем.
