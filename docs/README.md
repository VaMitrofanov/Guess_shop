...
12.08.2026 аватары Roblox в ЛК проходят через ограниченный серверный proxy: он по Roblox
user ID сам получает официальный headshot и отдаёт его без перекодирования. Это обходит
неисправный production `/_next/image` (500 для валидных PNG) и не даёт клиенту подставить
произвольный URL; при ошибке карточка показывает штатную иконку профиля.
