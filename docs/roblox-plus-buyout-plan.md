...
| SG single-flight contention | **Починен 17.07** | `gp-live-check` переведён на public API — больше не держит SG лок. Batch STOP_RE теперь ловит `BROWSER_BUSY` и останавливает пачку вместо бесполезных повторов. |
