...
`@@unique([orderId, gamepassId])` — один пасс дважды в заказе это двойное списание, а со
второго раза `AlreadyOwned` от Roblox. `onDelete: Cascade` — части живут только вместе с
заказом. У обычного заказа частей нет, и всё работает по единственному `WbOrder.gamepassId`;
