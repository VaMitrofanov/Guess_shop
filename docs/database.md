...
| `status` | `AVAILABLE → RESERVED → CLAIMED` (`WbCodeStatus`); `REVOKED` — тупик, см. ниже |
...
**`REVOKED`** (миграция `20260912_wb_code_revoked`) — заказ на WB отменён, деньги вернулись
покупателю, код больше не активируется нигде. Ставится **только** через
`bots/shared/wb-code-revocation.ts`, которое одновременно вешает на код заморозку
(`OrderHold`): статус закрывает дверь активации, заморозка — кассу выкупа. Стока `REVOKED`
не занимает: подсчёт остатков карточек считает `status = AVAILABLE`. Снятие — только
`scripts/revoke-gate.ts --release`, кнопки в консоли нет намеренно.
...
`WbMarketplaceOrder.claimOpenedAt` / `claimReason` (миграция `20260912_wb_buyer_claim`) —
заявка покупателя на возврат. Статусы DBS про неё **не знают вообще**: `XKFFJUU`
(WB `5722328333`) числился `receive/sold`, покупатель уже открыл заявку, а бот двое суток
слал напоминания по гейту. Источник — отдельный контур `returns-api.wildberries.ru`,
связь по `srid` = наш `rid`. Отметка останавливает напоминания (и гейтовые, и «нет геймпасса» в чат WB) и поднимает алерт.

`claimStatus` / `claimResolvedAt` (миграция `20260912_wb_code_revoked`) — **исход** заявки:
`status = 2` у WB значит «решение принято», `status_ex = 5` — деньги возвращаются. Без них
исход не виден ниоткуда: статус DBS-заказа меняется только в момент возврата денег, а между
решением и возвратом проходило от полутора суток (`XKFFJUU`) до восьми (`BJUM4MN`).
Время заявки `returns-api` отдаёт **московским и без указания зоны**, поэтому парсится оно
`wbClaimDate`, а не `safeDate`: в контейнере (UTC) заявка 17:48 МСК ложилась как 17:48Z.
...
Заморозка (30.08.2026, миграция `20260830_order_hold`): `heldAt` / `heldReason` / `heldBy` —
признак **поверх** статуса, «не выкупать, но и не удалять». Денормализация от `OrderHold`
(ниже), чтобы рабочие вкладки и автовыкуп фильтровались индексом `[heldAt]`, а не джойном
на горячем пути. Замороженный заказ исключён из всех очередей выкупа и из `restore-to-buyout`
/ `move-to`.

Ручной приоритет (03.09.2026, миграция `20260903_order_priority`): `priorityAt` / `priorityBy`
— признак поверх статуса, как заморозка, только наоборот: «выкупать первым». Хранится момент
нажатия, поэтому у нескольких поднятых первым идёт поднятый последним. Сортировать только как
`desc, nulls: "last"` — у `DESC` в Postgres по умолчанию `NULLS FIRST`, и без явного
`NULLS LAST` наверх уехали бы все НЕприоритетные заказы. Правило живёт одной строкой в
`src/lib/order-queue.ts` (`PRIORITY_ORDER_BY` / `PRIORITY_ORDER_SQL`), потому что очередь
читают четыре разных места: две ленты, голова «Обзора», выгрузка ID и автовыкуп бота.

Индексы покрывают все вкладки TWA (status+createdAt, favorites, purchaserUsername, orderSource,
robloxUsername, userId+createdAt, heldAt, priorityAt).

#### `OrderHold` — заморозка по коду (30.08.2026)
«Не выкупать, но и не удалять»: покупатель оставил 1 звезду, подозрение на фрод, открытый
спор. Раньше такие заказы кидали в `ERROR` — рабочий статус, где живёт кнопка «Вернуть к
выкупу»; одно нажатие возвращало заказ в очередь и тратило робуксы.

Поля: `wbCode` (**@unique**), `reason`, `createdBy`, `createdAt`, `releasedAt`, `releasedBy`.

**Ключ — код, а не `orderId`.** Код выдаётся покупателю раньше, чем бот создаёт заказ: случай
84CR7UZ — код в `RESERVED`, заказа ещё нет, а выкупать его уже нельзя. Заморозка по id
заставила бы ловить момент создания вручную; по коду она ставится заранее и садится на заказ
сама. `WbOrder.wbCode` уникален, поэтому у одной заморозки не больше одного заказа.

**Снятая заморозка не удаляется**, а помечается `releasedAt` — историю «кто и почему
заморозил» затирать нельзя, ради неё всё и делалось. Повторная заморозка того же кода —
`upsert` с обнулением `releasedAt`, а не вторая строка.

Синхронизация с заказом: крон `sweepPendingHolds` (раз в минуту в TG-боте) сажает активные
заморозки на заказы, созданные после них — покрывает все 11 мест, где рождается `WbOrder`,
не трогая ни одно. За безопасность траты он не отвечает: её держит `assertOrderNotHeld` прямо
в путях выкупа. Логика — `bots/shared/order-hold.ts` (единственный источник; веб
переэкспортирует через `src/lib/order-hold.ts`). Подробности —
[twa-admin.md](twa-admin.md#️-заморозка-заказа--не-выкупать-но-и-не-удалять-30082026).

#### `WbOrderGamepass` — части разбитого выкупа (28.08.2026)
Один заказ можно закрыть несколькими геймпассами: три пасса по 1000 R$ вместо одного на 3000.
Поля: `orderId`, `gamepassId`, `gamepassUrl`, `amount` (номинал ЧАСТИ), `position` (порядок
покупки), `chargedPrice` + `purchasedAt` (заполняются при выкупе части).

Зачем отдельная таблица, а не поле: прайс-гард сверяет цену пасса с номиналом, и до этой
таблицы номинал был один на заказ — пасс за 1429 R$ в заказе на 3000 не проходил гард никогда.

**Инвариант: сумма `amount` всех частей заказа равна `WbOrder.amount`.** Без допуска —
допуск означал бы, что покупатель получает не то количество робуксов, за которое заплатил.
Проверяется в коде при записи разбиения и повторно перед каждой покупкой.

**Один и тот же `gamepassId` может стоять в нескольких частях** (с 03.09.2026). У покупателя
сплошь и рядом выставлен один пасс на 1000, а заказ — на 2000: такой заказ раньше не
разбивался вовсе, потому что стоял `@@unique([orderId, gamepassId])`. Уникальность снята
миграцией `20260903_split_allow_repeated_gamepass`; повторы законны, потому что части
выкупаются с РАЗНЫХ доноров (в очереди аккаунты по 2-3 тысячи). Ограничение осталось одно и
проверяется в коде: у повторов один и тот же `amount` — цена у пасса одна.
`onDelete: Cascade` — части живут только вместе с заказом. У обычного заказа частей нет, и всё работает по единственному `WbOrder.gamepassId`;
`WbOrder.gamepassUrl` у разбитого заказа указывает на ТЕКУЩУЮ покупаемую часть, поэтому
очередь, поиск и уведомления не знают о разбиении вовсе. Подробности —
