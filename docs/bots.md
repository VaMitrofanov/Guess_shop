...
> ⚠️ **Этот способ оплаты нефискальный и планируется к замене.** Перевод по статическому
> QR не создаёт ни `PaymentAttempt`, ни чек, а подтверждение держится на визуальной проверке
> скриншота. После подключения эквайринга Т‑Банка правильный путь — та же официальная
> платёжка, что и на сайте: `Init` отдаёт `PaymentURL`, бот показывает его кнопкой.
> План и открытые вопросы владельца — [bot-acquiring-plan.md](bot-acquiring-plan.md),
> риск №32 в [security.md](security.md). Реализация не начата.
