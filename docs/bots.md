...
- **VK AWAITING_REVIEW тупик — ПОЧИНЕН 2026-07-18 (кейс RY1RYNT / vk1020787345).**
  Юзер нажал «📸 Отзыв = +100 R$» → стейт `AWAITING_REVIEW` → **каждый** последующий
...
- **VK COMPLETED-увед с кнопкой отзыва (П3, 2026-07-07):** корневая причина «фото
  ...
  `📸 Отзыв = +100 R$`). Фикс: inline keyboard с payload `review_hint` для первого
...
- **COMPLETED-карточки без WB-контента (П1, 2026-07-07):** после выкупа заказ
  ...
  текст про прямые заказы. Кнопка «📸 Отзыв = +100 R$» показывается пока бонус
...
- **Актуализация бонусных текстов (Ф3, 2026-07-12):** механика бонуса —
  `BONUS_MIN_PACK = 0` (добавка к **любому** номиналу прямого заказа, 30 дней,
  списывается автоматически) — эпоха «от 1000 R$» закрыта: 5 устаревших мест
  (crons.ts ×3, review-eligibility.ts, twa-notify.ts) переписаны на единую
  формулировку «Бонус добавится к любому прямому заказу автоматически — просто
  оформи(те) заказ в боте (без карточки WB)». `crons.ts` больше не дублирует
  ...
  `src/lib/twa-notify.ts` зеркалит значение с комментом-ссылкой). Свип-греп
  `от 1000`/`без карточки` по TG/VK/TWA/сайту — чисто.
