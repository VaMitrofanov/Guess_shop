...
**Имя «VK User»** в карточке — клиентский fallback из `VKAuthButton.tsx`: имя достаётся
декодом JWT `id_token` из `VKID.Auth.exchangeCode()`; если VK не вернул `id_token` или декод
упал, уходит `VK User`. Проблема периодическая (у одного юзера входы чередуются: то реальное
имя, то fallback). Побочный эффект: `authorize()` перезаписывает `User.name` при каждом
логине — неудачный декод затирает реальное имя в БД до следующего удачного входа
(~3 из 120 VK-юзеров «застряли» как `VK User` навсегда).
