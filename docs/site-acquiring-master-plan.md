...
> **Переехало.** Единая точка входа и полный остаток задач теперь в
> **[launch-roadmap.md](launch-roadmap.md)** — там актуальное состояние, порядок волн,
> внешние гейты и хвосты владельца. Список ниже сохранён как исторический (18.07) и
> частично закрыт; при расхождении верен roadmap.

<details>
<summary>Исторический порядок от 18.07</summary>
...
Новые замечания владельца имеют приоритет над этим порядком и должны добавляться в
[launch-roadmap.md](launch-roadmap.md) §4, а не теряться в отдельном списке.
...
С 2026-07-30 TG, VK и Web импортируют одну чистую функцию `retail-direct-v2`;
`/api/pricing`, калькулятор и server quote используют её же. Политика считает не старые
ступени, а непрерывную кривую желаемой суммы после УСН и эквайринга. Точки кривой:

| Пакет | Цена |
|---:|---:|
| 1 / 10 / 50 R$ | 3 / 2 / 1,6 ₽ за R$ |
| 100 R$ | 1,3 ₽ за R$ |
| 500 R$ | 1 ₽ за R$ |
| 1000 R$ | 0,9 ₽ за R$ |
| 3000 R$ | 0,8 ₽ за R$ |
...
Между точками ставка линейно интерполируется. Это внутренняя целевая сумма до
себестоимости Robux, а не клиентский курс и не чистая прибыль. Цена клиента решает обратную
задачу: из платежа вычитаются УСН «Доходы» 6% и консервативная обычная комиссия оплаты
`max(3,49 ₽; 3,49%)`; «Долями» не входит. Результат округляется вверх до целого рубля.
Клиенту в Web/TG/VK показывается фактический курс `итог к оплате ÷ получаемые R$`, включая
его бонус/рублёвую скидку. Пример: 500 R$ → целевые 500 ₽ → клиент платит 553 ₽ → курс
1,106 ₽/R$. Минимальная цена в карточке главной вычисляется той же функцией, что и
калькулятор, без отдельного хардкода. Клиентский UI не раскрывает состав внутренних
вычетов — УСН и эквайринг остаются в админской экономике.
...
Обязательные contract-тесты: все anchor-точки кривой, непрерывность/монотонность,
гарантия `платёж − УСН − эквайринг ≥ target net` для каждого допустимого целого номинала,
а также персональный бонус, скидка и истёкшая котировка.
